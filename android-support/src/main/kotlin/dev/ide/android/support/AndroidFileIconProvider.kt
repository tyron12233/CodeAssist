 ...
 * answers for the targets it recognises — the `AndroidManifest.xml`, the `res/` and `assets/` source
 * roots, and Android modules — and returns null for everything else so the default provider handles it.
 * The icon ids here (`manifest`, `sourceset.android-res`, `sourceset.assets`, `module.android`) are
 * matched by the UI's `TreeIcons` registry.
 */
object AndroidFileIconProvider : FileIconProvider {
    ...
    override fun iconFor(target: IconTarget): String? = when (target) {
        is IconTarget.File -> if (target.fileName == "AndroidManifest.xml") "manifest" else null

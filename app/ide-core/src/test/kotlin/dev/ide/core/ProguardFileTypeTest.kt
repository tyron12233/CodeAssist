...
class ProguardFileTypeTest {
    ...
    // Block body (not `= withTempDir(...)`): an expression-bodied test whose last call returns a value gets a
    // non-Unit return type, and JUnit Jupiter then silently SKIPS it.
    @Test
    fun proguardFileGetsItsIconAndNoJavaDiagnostics() {
        withTempDir("ide-proguard-filetype") { dir ->

...
/**
 * Interpreter-mediated project-resource resolution for the DESKTOP Compose preview — the JVM counterpart to
 * :ide-android's `AndroidPreviewResources`. Without it `stringResource(R.string.x)`/`colorResource`/… crash the
 * desktop preview trying to load the bytecode-less synthetic `R`; the desktop host never had a resolver wired.
 *
 ...
 * [ProjectPreviewResources] as the value engine (recursive `@ref` chasing, colour/dimension parsing). Compose
 * value types (`Color`/`Dp`) are pre-built here (interp-compose can't name them). Drawables aren't rendered on
 * desktop (no on-device `android.graphics` decode) — [painter] returns null, so `painterResource` degrades.
 */
class DesktopPreviewResources(
    ...
    private val engine = ProjectPreviewResources(repo, density = effectiveDensity)
    ...
    /** Drawables aren't rendered in the desktop preview (no `android.graphics` decode) — `painterResource`
     *  degrades to the real dispatch rather than a fabricated painter. */

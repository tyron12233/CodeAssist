...
public class EditVectorDialogFragment extends DialogFragment {
	...
	public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
			...
			public void onClick(View view) {
				...
				if (!color.getText().toString().trim().startsWith("#")) {
					...
					try {
						...
						icon.setColorFilter(Color.parseColor(color.getText().toString().trim()),
		...
		builder.setPositiveButton("Create", (d, w) -> {
			...
			generateSvg2Vector(width.getText().toString(), height.getText().toString(), color.getText().toString(),

					iconPath, projectResourceDirectory);
		...
		try {
			...
			if (nodeList.getLength() > 0) {
				...
				Files.write(
						...
						Paths.get(new File(iconPath + new File(iconPath).getName().replace(".svg", ".xml")).toURI()),

						vectorText, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING);
	...
	private boolean validate(EditText name, EditText height, EditText width, EditText color, EditText path) {
		...
		if (name.getText().toString().trim().isEmpty() && name.getText().toString().trim().endsWith(".xml")

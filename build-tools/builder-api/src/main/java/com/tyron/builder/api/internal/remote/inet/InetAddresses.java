...
class InetAddresses {
    ...
    private void analyzeNetworkInterface(NetworkInterface networkInterface) {
        logger.info("Adding IP addresses for network interface " + networkInterface.getDisplayName());
        try {
            boolean isLoopbackInterface = networkInterface.isLoopback();
            logger.info("Is this a loopback interface? " + isLoopbackInterface);
            ...
            while (candidates.hasMoreElements()) {
                ...
                if (isLoopbackInterface) {
                    if (candidate.isLoopbackAddress()) {
                        logger.info("Adding loopback address " + candidate);
                        loopback.add(candidate);
                    } else {
                        logger.info("Ignoring remote address on loopback interface " + candidate);
                    }
                } else {
                    if (candidate.isLoopbackAddress()) {
                        logger.info("Ignoring loopback address on remote interface " + candidate);
                    } else {
                        logger.info("Adding remote address " + candidate);
            ...
            logger.info("Error while querying interface " +
                        networkInterface +
                        " for IP addresses. " +
                        e);
        } catch (Throwable e) {

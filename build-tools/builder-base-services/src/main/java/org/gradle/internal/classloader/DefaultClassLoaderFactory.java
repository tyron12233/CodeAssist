    ...
    protected ClassLoader doCreateClassLoader(String name, ClassLoader parent, ClassPath classPath) {
        if (TestUtil.isDalvik()) {
            return GUtil.uncheckedCall(() -> {
                Class<?> classLoader = Class.forName("com.tyron.groovy.DexBackedURLClassLoader");
                Constructor<?> constructor =
                        classLoader.getConstructor(String.class, ClassLoader.class,
                                ClassPath.class);
                return (ClassLoader) constructor.newInstance(name, parent, classPath);
            });
        }

...
import com.tyron.builder.api.internal.collections.DefaultCollectionEventRegister;
import com.tyron.builder.api.internal.collections.ElementSource;
import com.tyron.builder.api.internal.collections.FilteredCollection;
import com.tyron.builder.api.internal.provider.CollectionProviderInternal;
import com.tyron.builder.api.internal.provider.DefaultListProperty;
...
public class DefaultDomainObjectCollection<T> extends AbstractCollection<T> implements DomainObjectCollection<T>, WithEstimatedSize, WithMutationGuard {
    ...
    protected CollectionFilter<T> createFilter(Predicate<? super T> filter) {
    ...
    @Override
    public DomainObjectCollection<T> matching(final Predicate<? super T> spec) {
    ...
    protected <S extends Collection<? super T>> S findAll(Closure cl, S matches) {
        ...
        for (T t : filteredStore(createFilter(Predicates.<Object>convertClosureToSpec(cl)))) {
            matches.add(t);
        }
        return matches;
    }

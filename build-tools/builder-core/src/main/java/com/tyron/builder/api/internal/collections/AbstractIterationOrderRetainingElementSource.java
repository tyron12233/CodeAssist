...
import com.tyron.builder.api.internal.provider.Collector;
...
import com.tyron.builder.api.internal.provider.Collectors.SingleElement;
...
abstract public class AbstractIterationOrderRetainingElementSource<T> implements ElementSource<T> {
    ...
    protected class RealizedElementCollectionIterator implements Iterator<T> {
        ...
        final Predicate<ValuePointer<T>> acceptanceSpec;
        int nextIndex = -1;
        int nextSubIndex = -1;
        int previousIndex = -1;
        ...
        RealizedElementCollectionIterator(List<Element<T>> backingList, Predicate<ValuePointer<T>> acceptanceSpec) {

...
public class DefaultTaskClassInfoStore implements TaskClassInfoStore {
    ...
    private final Function<Class<?>, TaskClassInfo> taskClassInfoFactory =
            aClass -> createTaskClassInfo(aClass.asSubclass(Task.class));
    ...
    private TaskClassInfo createTaskClassInfo(Class<? extends Task> type) {
        ...
        Optional<String> reasonNotToTrackState =
                Optional.ofNullable(type.getAnnotation(UntrackedTask.class))
                        .map(UntrackedTask::because);
        Map<String, Class<?>> processedMethods = Maps.newHashMap();
        ImmutableList.Builder<TaskActionFactory> taskActionFactoriesBuilder =
        ...
        for (Class current = type; current != null; current = current.getSuperclass()) {
            for (Method method : current.getDeclaredMethods()) {
                ...
                Class<?> previousDeclaringClass =
                        processedMethods.put(method.getName(), declaringClass);
                if (taskActionFactory instanceof AbstractIncrementalTaskActionFactory &&
                    foundIncrementalTaskActionFactory != null &&
                    method.getName()
                            .equals(foundIncrementalTaskActionFactory.getMethod().getName())) {
                    // Let's try if we can decide on one
                    AbstractIncrementalTaskActionFactory selectedTaskAction =
                            selectIncrementalTaskAction(type, foundIncrementalTaskActionFactory,
                                    taskActionFactory, current, method);
                ...
                if (taskActionFactory instanceof AbstractIncrementalTaskActionFactory) {
                    if (foundIncrementalTaskActionFactory != null) {
                        @SuppressWarnings("deprecation") Class<?> incrementalTaskInputsClass =
                                com.tyron.builder.api.tasks.incremental.IncrementalTaskInputs.class;
                        throw new BuildException(String.format(
                                "Cannot have multiple @TaskAction methods accepting an %s or %s " +
                                "parameter.",
                                InputChanges.class.getSimpleName(),
                                incrementalTaskInputsClass.getSimpleName()));
                    }
                    foundIncrementalTaskActionFactory =
                            (AbstractIncrementalTaskActionFactory) taskActionFactory;
                    continue;
                }
     ...
     * - If both task actions are of the same type (InputChanges or IncrementalTaskInputs), we
     * select the one we already found.
     * This happens when a subclass overrides the task action method in the subclass, while both
     * have TaskAction annotations.
     * - If the InputChanges is not deprecated and the IncrementalTaskInputs method is, then we
     * try to select one:
     * - If a subclass overrides the IncrementalTaskInputs method, we create a
     * {@link BridgingIncrementalInputsTaskActionFactory} which passes the {@link InputChanges}
     * object into the IncrementalTaskInputs method
     * - If no subclass overrides the IncrementalTaskInputs method, we use the InputChanges
     * method directly.
     * <p>
     ...
     */
    @Nullable
    private AbstractIncrementalTaskActionFactory selectIncrementalTaskAction(Class<? extends Task> taskClass,
                                                                             AbstractIncrementalTaskActionFactory foundTaskActionFactory,
                                                                             TaskActionFactory currentTaskActionFactory,
                                                                             Class currentClass,
                                                                             Method currentMethod) {
        if (currentTaskActionFactory.getClass() == foundTaskActionFactory.getClass()) {
            return foundTaskActionFactory;
        }
        AbstractIncrementalTaskActionFactory currentIncrementalTaskActionFactory =
                (AbstractIncrementalTaskActionFactory) currentTaskActionFactory;
        Map<Boolean, List<AbstractIncrementalTaskActionFactory>> partitionedActions =
                Stream.of(foundTaskActionFactory, currentIncrementalTaskActionFactory).collect(
                        Collectors.partitioningBy(
                                IncrementalInputsTaskActionFactory.class::isInstance));
        List<AbstractIncrementalTaskActionFactory> incrementalInputsFactories =
                partitionedActions.get(true);
        List<AbstractIncrementalTaskActionFactory> incrementalTaskInputFactories =
                partitionedActions.get(false);

        if (incrementalInputsFactories.size() == 1 && incrementalTaskInputFactories.size() == 1) {
            IncrementalInputsTaskActionFactory incrementalInputsTaskActionFactory =
                    (IncrementalInputsTaskActionFactory) Iterables
                            .getOnlyElement(incrementalInputsFactories);
            IncrementalTaskInputsTaskActionFactory incrementalTaskInputsTaskActionFactory =
                    (IncrementalTaskInputsTaskActionFactory) Iterables
                            .getOnlyElement(incrementalTaskInputFactories);

            if (isDeprecated(incrementalTaskInputsTaskActionFactory) &&
                !isDeprecated(incrementalInputsTaskActionFactory)) {
                Class<?> declaringClassForIncrementalTaskInputsMethod =
                        getDeclaringClassForIncrementalTaskInputsMethod(taskClass,
    ...
    private Class<?> getDeclaringClassForIncrementalTaskInputsMethod(Class<? extends Task> type,
                                                                     String methodName) {
    ...
    private boolean isDeprecated(AbstractIncrementalTaskActionFactory foundIncrementalTaskActionFactory) {
        return foundIncrementalTaskActionFactory.getMethod().getAnnotation(Deprecated.class) !=
               null;
    }

    @Nullable
    private static TaskActionFactory createTaskAction(Class<? extends Task> taskType,
                                                      final Method method) {
    ...
    private static class IncrementalInputsTaskActionFactory extends AbstractIncrementalTaskActionFactory {
        ...
        @Override
        protected Action<? super Task> doCreate(Instantiator instantiator,
    ...
    private static class IncrementalTaskInputsTaskActionFactory extends AbstractIncrementalTaskActionFactory {
        public IncrementalTaskInputsTaskActionFactory(Class<? extends Task> taskType,
                                                      Method method) {
            super(taskType, method);
        }
    ...
    private static class BridgingIncrementalInputsTaskActionFactory extends AbstractIncrementalTaskActionFactory {
        ...
        public BridgingIncrementalInputsTaskActionFactory(Class<? extends Task> taskType,
                                                          Method method) {
            super(taskType, method);
        }
    ...
    private static abstract class AbstractIncrementalTaskActionFactory implements TaskActionFactory {
        ...
        protected abstract Action<? super Task> doCreate(Instantiator instantiator,
                                                         Class<? extends Task> taskType,
                                                         Method method);

...
public class DefaultBuildOperationExecutor implements BuildOperationExecutor, Stoppable {
    ...
    public DefaultBuildOperationExecutor(BuildOperationListener listener,
                                         Clock clock,
        ...
        this.runner = new DefaultBuildOperationRunner(currentBuildOperationRef, clock::getCurrentTime,
        ...
        managedExecutors.put(BuildOperationConstraint.UNCONSTRAINED, executorFactory.create("Unconstrained build operations",
                parallelismConfiguration.getMaxWorkerCount() * 10));
    }
    ...
    @Override
    public <O extends BuildOperation> void execute(O buildOperation,
                                                   BuildOperationWorker<O> worker,
                                                   @Nullable BuildOperationState defaultParent) {
    ...
    public <O extends RunnableBuildOperation> void runAll(Action<BuildOperationQueue<O>> schedulingAction,
        ...
        executeInParallel(false, new QueueWorker<>(getCurrentBuildOperation(), RunnableBuildOperation::run),
                schedulingAction, buildOperationConstraint);
    }
    ...
    public <O extends RunnableBuildOperation> void runAllWithAccessToProjectState(Action<BuildOperationQueue<O>> schedulingAction,
                                                                                  BuildOperationConstraint buildOperationConstraint) {
        executeInParallel(true, new QueueWorker<>(getCurrentBuildOperation(), RunnableBuildOperation::run),
                schedulingAction, buildOperationConstraint);
    }
    ...
    @Override
    public <O extends BuildOperation> void runAll(BuildOperationWorker<O> worker,
                                                  Action<BuildOperationQueue<O>> schedulingAction,
                                                  BuildOperationConstraint buildOperationConstraint) {
        executeInParallel(false, new QueueWorker<>(getCurrentBuildOperation(), worker),
                schedulingAction, buildOperationConstraint);
    }
    ...
    private <O extends BuildOperation> void executeInParallel(boolean allowAccessToProjectState,
                                                              BuildOperationQueue.QueueWorker<O> worker,
    ...
    private static class ListenerAdapter implements DefaultBuildOperationRunner.BuildOperationExecutionListener {
        ...
        public ListenerAdapter(BuildOperationListener buildOperationListener,
                               ProgressLoggerFactory progressLoggerFactory,
                               Clock clock) {
        ...
        @Override
        public void progress(BuildOperationDescriptor descriptor,
                             long progress,
                             long total,
                             String units,
                             String status) {
            progress(descriptor, status);
            buildOperationListener.progress(descriptor.getId(),
                    new OperationProgressEvent(clock.getCurrentTime(), new OperationProgressDetails(progress, total, units)));
        }

        @Override
        public void stop(BuildOperationDescriptor descriptor,
            ...
            buildOperationListener.finished(descriptor,
                    new OperationFinishEvent(operationState.getStartTime(), clock.getCurrentTime(),
                            context.getFailure(), context.getResult()));
        }

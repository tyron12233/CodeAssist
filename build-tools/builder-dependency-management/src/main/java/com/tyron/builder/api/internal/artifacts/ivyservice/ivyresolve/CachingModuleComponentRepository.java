...
import com.tyron.builder.api.internal.artifacts.ivyservice.modulecache.ModuleMetadataCache;
import com.tyron.builder.api.internal.artifacts.ivyservice.modulecache.ModuleRepositoryCaches;
import com.tyron.builder.api.internal.artifacts.ivyservice.modulecache.artifacts.ArtifactAtRepositoryKey;
import com.tyron.builder.api.internal.artifacts.ivyservice.modulecache.artifacts.CachedArtifact;
import com.tyron.builder.api.internal.artifacts.ivyservice.modulecache.artifacts.CachedArtifacts;
import com.tyron.builder.api.internal.artifacts.ivyservice.modulecache.artifacts.ModuleArtifactCache;
import com.tyron.builder.api.internal.artifacts.ivyservice.modulecache.artifacts.ModuleArtifactsCache;
import com.tyron.builder.api.internal.artifacts.ivyservice.modulecache.dynamicversions.ModuleVersionsCache;
...
import com.tyron.builder.internal.component.external.model.FixedComponentArtifacts;
...
import com.tyron.builder.internal.resolve.ArtifactNotFoundException;
...
/**
 * A ModuleComponentRepository that loads and saves resolution results in the dependency
 * resolution cache.
 * <p>
 * The `LocateInCacheRepositoryAccess` provided by {@link #getLocalAccess()} will attempt to
 * handle any resolution request
 * directly from the cache, checking for cache expiry based on the `ResolutionStrategy` in
 * operation.
 * <p>
 * The `ResolveAndCacheRepositoryAccess` provided by {@link #getRemoteAccess()} will first
 * delegate any resolution request,
...
public class CachingModuleComponentRepository implements ModuleComponentRepository {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(CachingModuleComponentRepository.class);
    ...
    private final LocateInCacheRepositoryAccess locateInCacheRepositoryAccess =
            new LocateInCacheRepositoryAccess();
    ...
    private class LocateInCacheRepositoryAccess implements ModuleComponentRepositoryAccess {
        ...
        public void listModuleVersions(ModuleDependencyMetadata dependency,
            ...
            // First try to determine the artifacts in-memory (e.g using the metadata): don't use
        ...
        private void listModuleVersionsFromCache(ModuleDependencyMetadata dependency,
                                                 BuildableModuleVersionListingResolveResult result) {
            ...
            if (cachedModuleVersionList != null) {
                ...
                Expiry expiry = cachePolicy
                        .versionListExpiry(moduleId, versions, cachedModuleVersionList.getAge());
                if (expiry.isMustCheck()) {
                    LOGGER.debug(
                            "Version listing in dynamic revision cache is expired: will perform " +
                            "fresh resolve of '{}' in '{}'",
                            requested, delegate.getName());
                } else {
                    // When age == 0, verified since the start of this build, assume listing
                    // hasn't changed
        ...
        public void resolveComponentMetaData(ModuleComponentIdentifier moduleComponentIdentifier,
            ...
            // First try to determine the artifacts in-memory (e.g using the metadata): don't use
            // the cache in this case
            delegate.getLocalAccess()
                    .resolveComponentMetaData(moduleComponentIdentifier, requestMetaData, result);
            if (result.hasResult()) {
                return;
            }
        ...
        private void resolveComponentMetaDataFromCache(ModuleComponentIdentifier moduleComponentIdentifier,
                                                       ComponentOverrideMetadata requestMetaData,
                                                       BuildableModuleComponentMetaDataResolveResult result) {
            ModuleMetadataCache.CachedMetadata cachedMetadata = moduleMetadataCache
                    .getCachedModuleDescriptor(delegate, moduleComponentIdentifier);
            ...
            if (cachedMetadata.isMissing()) {
                if (cachePolicy
                        ...
                        .isMustCheck()) {
                    LOGGER.debug(
                            "Cached meta-data for missing module is expired: will perform fresh " +
                            "resolve of '{}' in '{}'",
                            moduleComponentIdentifier, delegate.getName());
                    return;
                }
                LOGGER.debug("Detected non-existence of module '{}' in resolver cache '{}'",
                        moduleComponentIdentifier, delegate.getName());
                result.missing();
            ...
            if (requestMetaData.isChanging() || metadata.isChanging()) {
                Expiry expiry = cachePolicy.changingModuleExpiry(moduleComponentIdentifier,
                        cachedMetadata.getModuleVersion(), cachedMetadata.getAge());
                if (expiry.isMustCheck()) {
                    LOGGER.debug(
                            "Cached meta-data for changing module is expired: will perform fresh " +
                            "resolve of '{}' in '{}'",
                            moduleComponentIdentifier, delegate.getName());
                    return;
                }
                LOGGER.debug("Found cached version of changing module '{}' in '{}'",
                        moduleComponentIdentifier, delegate.getName());
                listener.onChangingModuleResolve(moduleComponentIdentifier, expiry);
            } else {
                if (cachePolicy
                        .moduleExpiry(moduleComponentIdentifier, cachedMetadata.getModuleVersion(),
                                cachedMetadata.getAge()).isMustCheck()) {
                    LOGGER.debug(
                            "Cached meta-data for module must be refreshed: will perform fresh " +
                            "resolve of '{}' in '{}'",
                            moduleComponentIdentifier, delegate.getName());
                    return;
                }
            ...
            LOGGER.debug("Using cached module metadata for module '{}' in '{}'",
                    moduleComponentIdentifier, delegate.getName());
            result.resolved(metadata);
            // When age == 0, verified since the start of this build, assume the meta-data hasn't
            // changed
            result.setAuthoritative(cachedMetadata.getAge().toMillis() == 0);
        }

        private ModuleComponentResolveMetadata getProcessedMetadata(int key,
        ...
        @Override
        public void resolveArtifactsWithType(ComponentResolveMetadata component,
        ...
        @Override
        public void resolveArtifacts(ComponentResolveMetadata component,
            ...
            DefaultBuildableArtifactSetResolveResult artifactsResolveResult =
                    new DefaultBuildableArtifactSetResolveResult();
        ...
        private void resolveModuleArtifactsFromCache(String contextId,
            ...
            if (cachedModuleArtifacts != null) {
                if (!cachePolicy.moduleArtifactsExpiry(component.getModuleVersionId(), null,
                ...
                LOGGER.debug(
                        "Artifact listing has expired: will perform fresh resolve of '{}' for " +
                        "'{}' in '{}'",
                        contextId, component.getModuleVersionId(), delegate.getName());
            }
        }

        @Override
        public void resolveArtifact(ComponentArtifactMetadata artifact,
        ...
        public MetadataFetchingCost estimateMetadataFetchingCost(ModuleComponentIdentifier moduleComponentIdentifier) {
            ...
            if (metaData.isChanging()) {
                if (cachePolicy.changingModuleExpiry(moduleComponentIdentifier,
                        cachedMetadata.getModuleVersion(), cachedMetadata.getAge()).isMustCheck()) {
                    return estimateCostViaRemoteAccess(moduleComponentIdentifier);
                }
            } else {
                if (cachePolicy
        ...
        private MetadataFetchingCost estimateCostViaRemoteAccess(ModuleComponentIdentifier moduleComponentIdentifier) {
            return delegate.getRemoteAccess()
                    .estimateMetadataFetchingCost(moduleComponentIdentifier);
        }

        private void resolveArtifactFromCache(ComponentArtifactMetadata artifact,
                                              ModuleSources moduleSources,
                                              BuildableArtifactResolveResult result) {
            CachedArtifact cached = moduleArtifactCache.lookup(artifactCacheKey(artifact.getId()));
            if (cached != null) {
                ModuleDescriptorHashModuleSource moduleSource =
                        findCachingModuleSource(moduleSources);
                final HashCode descriptorHash = moduleSource.getDescriptorHash();
                Duration age =
                        Duration.ofMillis(timeProvider.getCurrentTime() - cached.getCachedAt());
                ...
                ArtifactIdentifier artifactIdentifier =
                        moduleComponentArtifactMetadata.toArtifactIdentifier();
                if (cached.isMissing()) {
                    Expiry expiry = cachePolicy
                            .artifactExpiry(artifactIdentifier, null, age, isChangingModule,
                                    descriptorHash.equals(cached.getDescriptorHash()));
                    if (!expiry.isMustCheck()) {
                        LOGGER.debug("Detected non-existence of artifact '{}' in resolver cache",
                                artifact);
                    ...
                    Expiry expiry = cachePolicy
                            .artifactExpiry(artifactIdentifier, cachedArtifactFile, age,
                                    isChangingModule,
                                    descriptorHash.equals(cached.getDescriptorHash()));
                    if (!expiry.isMustCheck()) {
                        LOGGER.debug("Found artifact '{}' in resolver cache: {}", artifact,
                                cachedArtifactFile);
    ...
    private ModuleDescriptorHashModuleSource findCachingModuleSource(ModuleSources sources) {
        return sources.getSource(ModuleDescriptorHashModuleSource.class).orElseThrow(
                () -> new RuntimeException("Cannot find expected module source " +
                                           ModuleDescriptorHashModuleSource.class.getSimpleName() +
                                           " in " +
                                           sources));
    }
    ...
    private class ResolveAndCacheRepositoryAccess implements ModuleComponentRepositoryAccess {
        ...
        public void listModuleVersions(ModuleDependencyMetadata dependency,
            ...
            switch (result.getState()) {
                case Listed:
                    ...
                    Set<ModuleVersionIdentifier> versions = versionList.stream()
                            .map(original -> DefaultModuleVersionIdentifier
                                    .newId(moduleId, original)).collect(Collectors.toSet());
                    moduleVersionsCache.cacheModuleVersionList(delegate, moduleId, versionList);
                    listener.onDynamicVersionSelection(dependency.getSelector(),
                ...
                default:
                    throw new IllegalStateException(
                            "Unexpected state on listModuleVersions: " + result.getState());
            }
        }
        ...
        public void resolveComponentMetaData(ModuleComponentIdentifier moduleComponentIdentifier,
            ...
            delegate.getRemoteAccess()
                    .resolveComponentMetaData(moduleComponentIdentifier, forced, result);
            switch (result.getState()) {
                case Missing:
                ...
                case Resolved:
                    ...
                    ModuleMetadataCache.CachedMetadata cachedMetadata = moduleMetadataCache
                            .cacheMetaData(delegate, moduleComponentIdentifier, resolvedMetadata);
                    // Starting here we're going to process the component metadata rules
                    // Therefore metadata can be mutated, and will _not_ be stored in the module
                    // metadata cache
                    ...
                    if (processedMetadata.isChanging() || requestMetaData.isChanging()) {
                        ...
                        Expiry expiry = cachePolicy.changingModuleExpiry(moduleComponentIdentifier,
                                cachedMetadata.getModuleVersion(), Duration.ZERO);
                        listener.onChangingModuleResolve(moduleComponentIdentifier, expiry);
                    }
                    cachedMetadata.putProcessedMetadata(metadataProcessor.getRulesHash(),
                            processedMetadata);
                ...
                default:
                    throw new IllegalStateException(
                            "Unexpected resolve state: " + result.getState());
            }
        }

        private ModuleComponentResolveMetadata makeChanging(ModuleComponentResolveMetadata resolvedMetadata,
                                                            ModuleComponentResolveMetadata processedMetadata) {
            ...
            resolvedMetadata.getSources().withSources(src -> {
                if (src instanceof ModuleDescriptorHashModuleSource) {
                    ModuleDescriptorHashModuleSource changingSource =
                            new ModuleDescriptorHashModuleSource(
        ...
        public void resolveArtifactsWithType(ComponentResolveMetadata component,
            ...
            if (result.getFailure() == null) {
                ModuleDescriptorHashModuleSource moduleSource =
                        findCachingModuleSource(component.getSources());
                moduleArtifactsCache
                        .cacheArtifacts(delegate, component.getId(), cacheKey(artifactType),
                                moduleSource.getDescriptorHash(), result.getResult());
            }
        }
        ...
        public void resolveArtifacts(ComponentResolveMetadata component,
            ...
            if (result.getFailure() == null) {
                ...
                moduleArtifactsCache.cacheArtifacts(delegate, component.getId(), "component:",
                        moduleSource.getDescriptorHash(), artifacts.getArtifacts());
            }
        }
        ...
        public void resolveArtifact(ComponentArtifactMetadata artifact,
            ...
            LOGGER.debug("Downloaded artifact '{}' from resolver: {}", artifact,
                    delegate.getName());

            ArtifactResolveException failure = result.getFailure();
            ModuleDescriptorHashModuleSource cachingModuleSource =
                    findCachingModuleSource(moduleSources);
            if (failure == null) {
                moduleArtifactCache.store(artifactCacheKey(artifact.getId()), result.getResult(),
                        cachingModuleSource.getDescriptorHash());
            } else if (failure instanceof ArtifactNotFoundException) {
                moduleArtifactCache
                        .storeMissing(artifactCacheKey(artifact.getId()), result.getAttempted(),
                                cachingModuleSource.getDescriptorHash());
            }
        }

...
import com.tyron.builder.internal.resolve.RejectedBySelectorVersion;
...
import com.tyron.builder.internal.resolve.result.DefaultBuildableModuleVersionListingResolveResult;
...
import static com.tyron.builder.internal.resolve.result.BuildableModuleComponentMetaDataResolveResult.State.Failed;
import static com.tyron.builder.internal.resolve.result.BuildableModuleComponentMetaDataResolveResult.State.Resolved;
    ...
    private RepositoryChainModuleResolution findLatestModule(LinkedList<RepositoryResolveState> queue, Collection<Throwable> failures, Collection<RepositoryResolveState> missing) {
        ...
        while (!queue.isEmpty()) {
            ...
            try {
                ...
                failures.add(t);
                if (ResolveExceptionAnalyzer.isCriticalFailure(t)) {
                    queue.clear();
                }
                continue;
            }
            switch (request.resolvedVersionMetadata.getState()) {
                case Failed:
                    ...
                    if (ResolveExceptionAnalyzer.isCriticalFailure(request.resolvedVersionMetadata.getFailure())) {
                        queue.clear();
                    }
                    break;
                case Missing:

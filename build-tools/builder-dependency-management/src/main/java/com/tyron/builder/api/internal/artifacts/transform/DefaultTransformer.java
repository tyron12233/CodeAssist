...
import static com.tyron.builder.api.internal.tasks.properties.AbstractValidatingProperty.reportValueNotSet;
...
import com.tyron.builder.api.artifacts.transform.VariantTransformConfigurationException;
...
public class DefaultTransformer extends AbstractTransformer<TransformAction<?>> {
    ...
    private final CalculatedValueContainer<IsolatedParameters, IsolateTransformerParameters>
            isolatedParameters;
    ...
    public DefaultTransformer(Class<? extends TransformAction<?>> implementationClass,
        ...
        this.requiresDependencies = instanceFactory
                .serviceInjectionTriggeredByAnnotation(InputArtifactDependencies.class);
        ...
        this.isolatedParameters = calculatedValueContainerFactory
                .create(Describables.of("parameters of", this),
                        new IsolateTransformerParameters(parameterObject, implementationClass,
                                cacheable, owner, parameterPropertyWalker, isolatableFactory,
                                buildOperationExecutor, classLoaderHierarchyHasher,
    ...
    public DefaultTransformer(Class<? extends TransformAction<?>> implementationClass,
                              CalculatedValueContainer<IsolatedParameters,
                                      IsolateTransformerParameters> isolatedParameters,
                              ImmutableAttributes fromAttributes,
                              Class<? extends FileNormalizer> inputArtifactNormalizer,
                              Class<? extends FileNormalizer> dependenciesNormalizer,
                              boolean cacheable,
    ...
    public static void validateInputFileNormalizer(String propertyName,
                                                   @Nullable Class<? extends FileNormalizer> normalizer,
                                                   boolean cacheable,
                                                   TypeValidationContext validationContext) {
        if (cacheable) {
            if (normalizer == AbsolutePathInputNormalizer.class) {
                validationContext.visitPropertyProblem(problem -> problem
                        .withId(ValidationProblemId.CACHEABLE_TRANSFORM_CANT_USE_ABSOLUTE_SENSITIVITY)
                        .reportAs(Severity.ERROR).forProperty(propertyName)
                        .withDescription("is declared to be sensitive to absolute paths")
                        .happensBecause("This is not allowed for cacheable transforms")
                        .withLongDescription(
                                "Absolute path sensitivity does not allow sharing the transform " +
                                "result between different machines, although that is the goal of " +
                                "cacheable transforms.").addPossibleSolution(
                                "Use a different normalization strategy via @PathSensitive, " +
                                "@Classpath or @CompileClasspath")
                        .documentedAt("validation_problems",
    ...
    @Override
    public ImmutableList<File> transform(Provider<FileSystemLocation> inputArtifactProvider,
                                         File outputDir,
                                         ArtifactTransformDependencies dependencies,
                                         @Nullable InputChanges inputChanges) {
        TransformAction<?> transformAction =
                newTransformAction(inputArtifactProvider, dependencies, inputChanges);
        DefaultTransformOutputs transformOutputs =
                new DefaultTransformOutputs(inputArtifactProvider.get().getAsFile(), outputDir,
                        fileLookup);
    ...
    private static void fingerprintParameters(DocumentationRegistry documentationRegistry,
        ...
        DefaultTypeValidationContext validationContext =
                DefaultTypeValidationContext.withoutRootType(documentationRegistry, cacheable);
        InputFingerprinter.Result result = inputFingerprinter
                .fingerprintInputProperties(ImmutableSortedMap.of(), ImmutableSortedMap.of(),
                        ImmutableSortedMap.of(), ImmutableSortedMap.of(), visitor -> propertyWalker
                                .visitProperties(parameterObject, validationContext,
                                        new PropertyVisitor.Adapter() {
                                            @Override
                                            public void visitInputProperty(String propertyName,
                                                                           PropertyValue value,
                                                                           boolean optional) {
                                                try {
                                                    Object preparedValue = InputParameterUtils
                                                            .prepareInputParameterValue(value);

                                                    if (preparedValue == null && !optional) {
                                                        reportValueNotSet(propertyName,
                                                                validationContext);
                                                    }
                                                    visitor.visitInputProperty(propertyName,
                                                            () -> preparedValue);
                                                } catch (Throwable e) {
                                            ...
                                            public void visitInputFileProperty(String propertyName,
                                                                               ...
                                                                               boolean incremental,
                                                                               @Nullable Class<?
                                                                                       extends FileNormalizer> fileNormalizer,
                                                                               PropertyValue value,
                                                                               InputFilePropertyType filePropertyType) {
                                                validateInputFileNormalizer(propertyName,
                                                        fileNormalizer, cacheable,
                                                        validationContext);
                                                visitor.visitInputFileProperty(propertyName,
                                                        incremental ?
                                                                InputFingerprinter.InputPropertyType.INCREMENTAL : InputFingerprinter.InputPropertyType.NON_INCREMENTAL,
                                                        new FileValueSupplier(value,
                                                                fileNormalizer ==
                                                                null ? AbsolutePathInputNormalizer.class : fileNormalizer,
                                                                directorySensitivity,
                                                                lineEndingNormalization,
                                                                () -> FileParameterUtils
                                                                        .resolveInputFileValue(
                                            ...
                                            @Override
                                            public void visitOutputFileProperty(String propertyName,
                                                                                boolean optional,
                                                                                PropertyValue value,
                                                                                OutputFilePropertyType filePropertyType) {
                                                validationContext.visitPropertyProblem(
                                                        problem -> problem
                                                                .withId(ValidationProblemId.ARTIFACT_TRANSFORM_SHOULD_NOT_DECLARE_OUTPUT)
        ...
        if (!validationMessages.isEmpty()) {
            throw new DefaultMultiCauseException(String.format(validationMessages.size() == 1 ?
                    "A problem was found with the " +
                    "configuration of the artifact" +
                    " transform parameter %s." :
                    "Some problems were found with the configuration of the artifact " +
                    "transform parameter %s.", getParameterObjectDisplayName(parameterObject)),
                    validationMessages.keySet().stream().sorted().map(InvalidUserDataException::new)
        ...
        for (Map.Entry<String, CurrentFileCollectionFingerprint> entry : result
                .getFileFingerprints().entrySet()) {
            hasher.putString(entry.getKey(), StandardCharsets.UTF_8);
            Hashes.putHash(hasher, entry.getValue().getHash());
        }
    }
    ...
    private TransformAction<?> newTransformAction(Provider<FileSystemLocation> inputArtifactProvider,
                                                  ArtifactTransformDependencies artifactTransformDependencies,
                                                  @Nullable InputChanges inputChanges) {
        TransformParameters parameters =
                isolatedParameters.get().getIsolatedParameterObject().isolate();
        ServiceLookup services =
                new IsolationScheme<>(TransformAction.class, TransformParameters.class,
                        TransformParameters.None.class)
                        .servicesForImplementation(parameters, internalServices);
        services = new TransformServiceLookup(inputArtifactProvider,
                requiresDependencies ? artifactTransformDependencies : null, inputChanges,
                services);
    ...
    private static class TransformServiceLookup implements ServiceLookup {
        private static final Type FILE_SYSTEM_LOCATION_PROVIDER =
                new TypeToken<Provider<FileSystemLocation>>() {
                }.getType();

        private final ImmutableList<InjectionPoint> injectionPoints;
        private final ServiceLookup delegate;

        public TransformServiceLookup(Provider<FileSystemLocation> inputFileProvider,
                                      @Nullable ArtifactTransformDependencies artifactTransformDependencies,
                                      @Nullable InputChanges inputChanges,
            ...
            builder.add(InjectionPoint.injectedByAnnotation(InputArtifact.class, File.class, () -> {
                DeprecationLogger.deprecate("Injecting the input artifact of a transform as a File")
                        .withAdvice("Declare the input artifact as Provider<FileSystemLocation> " +
                                    "instead.").willBecomeAnErrorInGradle8()
                        .withUserManual("artifact_transforms",
                                "sec:implementing-artifact-transforms").nagUser();
                return inputFileProvider.get().getAsFile();
            }));
            builder.add(InjectionPoint
                    .injectedByAnnotation(InputArtifact.class, FILE_SYSTEM_LOCATION_PROVIDER,
                            () -> inputFileProvider));
            if (artifactTransformDependencies != null) {
                builder.add(InjectionPoint.injectedByAnnotation(InputArtifactDependencies.class,
                        () -> artifactTransformDependencies.getFiles().orElseThrow(
                                () -> new IllegalStateException(
                                        "Transform does not use artifact dependencies."))));
            }
        ...
        public Object get(Type serviceType) throws UnknownServiceException, ServiceLookupException {
            ...
            if (result == null) {
                throw new UnknownServiceException(serviceType,
                        "No service of type " + serviceType + " available.");
            }
            return result;
        }
        ...
        private static class InjectionPoint {
            ...
            public static InjectionPoint injectedByAnnotation(Class<? extends Annotation> annotation,
                                                              Supplier<Object> valueToInject) {
                return new InjectionPoint(annotation, determineTypeFromAnnotation(annotation),
                        valueToInject);
            }
            ...
            public static InjectionPoint injectedByType(Class<?> injectedType,
                                                        Supplier<Object> valueToInject) {
                return new InjectionPoint(null, injectedType, valueToInject);
            }

            private InjectionPoint(@Nullable Class<? extends Annotation> annotation,
                                   Type injectedType,
                                   Supplier<Object> valueToInject) {
            ...
            private static Class<?> determineTypeFromAnnotation(Class<? extends Annotation> annotation) {
                ...
                if (supportedTypes.length != 1) {
                    throw new IllegalArgumentException(
                            "Cannot determine supported type for annotation " +
                            annotation.getName());
                }
    ...
    public static class IsolatedParameters {
        ...
        public IsolatedParameters(Isolatable<? extends TransformParameters> isolatedParameterObject,
                                  HashCode secondaryInputsHash) {
    ...
    public static class IsolateTransformerParameters implements ValueCalculator<IsolatedParameters> {
        ...
        public void visitDependencies(TaskDependencyResolveContext context) {
            if (parameterObject != null) {
                parameterPropertyWalker.visitProperties(parameterObject, TypeValidationContext.NOOP,
                        new PropertyVisitor.Adapter() {
                            @Override
                            public void visitInputFileProperty(String propertyName,
                                                               boolean optional,
        ...
        private IsolatedParameters isolateParameters(InputFingerprinter inputFingerprinter) {
            ...
            if (!model.hasMutableState()) {
                // This may happen when a task visits artifacts using a FileCollection instance
                // created from a Configuration instance in a different project (not an artifact
                // produced by a different project, these work fine)
                // There is a check in DefaultConfiguration that deprecates resolving
                // dependencies via FileCollection instance created by a different project,
                // however that check may not
                // necessarily be triggered. For example, the configuration may be legitimately
                // resolved by some other task prior to the problematic task running
                // TODO - hoist this up into configuration file collection visiting (and not when
                //  visiting the upstream dependencies of a transform), and deprecate this in
                //  Gradle 7.x
                //
                // This may also happen when a transform takes upstream dependencies and the
                // dependencies are transformed using a different transform
                // In this case, the main thread that schedules the work should isolate the
                // transform parameters prior to scheduling the work. However, the dependencies may
                // be filtered from the result, so that the transform is not visited by the main
                // thread, or the transform worker may start work before the main thread
                // has a chance to isolate the upstream transform
                // TODO - ensure all transform parameters required by a transform worker are
                //  isolated prior to starting the worker
                //
                // Force access to the state of the owner, regardless of whether any other thread
                // has access. This is because attempting to acquire a lock for a project may
                // deadlock
                // when performed from a worker thread (see DefaultBuildOperationQueue
                // .waitForCompletion() which intentionally does not release the project locks
                // while waiting)
                // TODO - add validation to fail eagerly when a worker attempts to lock a project
                //
                return model
                        .forceAccessToMutableState(o -> doIsolateParameters(inputFingerprinter));
            } else {
        ...
        private IsolatedParameters doIsolateParameters(InputFingerprinter inputFingerprinter) {
            try {
                ...
                formatter.node("Could not isolate parameters ").appendValue(parameterObject)
                        .append(" of artifact transform ").appendType(implementationClass);
        ...
        private IsolatedParameters isolateParametersExclusively(InputFingerprinter inputFingerprinter) {
            Isolatable<TransformParameters> isolatedParameterObject =
                    isolatableFactory.isolate(parameterObject);
            ...
            if (parameterObject != null) {
                ...
                buildOperationExecutor.run(new RunnableBuildOperation() {
                    ...
                    public void run(BuildOperationContext context) {
                        // TODO wolfs - schedule fingerprinting separately, it can be done
                        //  without having the project lock
                        fingerprintParameters(documentationRegistry, inputFingerprinter,
                                fileCollectionFactory, parameterPropertyWalker, hasher,
                                isolatedTransformParameters, cacheable);
    ...
    /*
     * This operation is only used here temporarily. Should be replaced with a more stable

...
import static com.tyron.builder.internal.execution.fingerprint.InputFingerprinter.InputPropertyType.INCREMENTAL;
import static com.tyron.builder.internal.execution.fingerprint.InputFingerprinter.InputPropertyType.NON_INCREMENTAL;
import static com.tyron.builder.internal.file.TreeType.DIRECTORY;
import static com.tyron.builder.internal.file.TreeType.FILE;
...
public class DefaultTransformerInvocationFactory implements TransformerInvocationFactory {
    private static final CachingDisabledReason NOT_CACHEABLE =
            new CachingDisabledReason(CachingDisabledReasonCategory.NOT_CACHEABLE,
                    "Caching not enabled.");
    ...
    public DefaultTransformerInvocationFactory(ExecutionEngine executionEngine,
    ...
    @Override
    public CacheableInvocation<ImmutableList<File>> createInvocation(Transformer transformer,
                                                                     File inputArtifact,
        ...
        TransformationWorkspaceServices workspaceServices =
                determineWorkspaceServices(producerProject);

        UnitOfWork execution;
        if (producerProject == null) {
            execution = new ImmutableTransformerExecution(transformer, inputArtifact, dependencies,
                    buildOperationExecutor, fileCollectionFactory, inputFingerprinter,
                    fileSystemAccess, workspaceServices);
        } else {
            execution = new MutableTransformerExecution(transformer, inputArtifact, dependencies,
                    buildOperationExecutor, fileCollectionFactory, inputFingerprinter,
                    workspaceServices);
        }

        return executionEngine.createRequest(execution)
                .withIdentityCache(workspaceServices.getIdentityCache()).getOrDeferExecution(
                        new DeferredExecutionHandler<TransformationResult,
                                CacheableInvocation<ImmutableList<File>>>() {
                            @Override
                            ...
                            @Override
                            public CacheableInvocation<ImmutableList<File>> processDeferredOutput(
                                    Supplier<Try<TransformationResult>> deferredExecution) {
                            ...
                            private Try<ImmutableList<File>> mapResult(Try<TransformationResult> cachedOutput) {
                                return cachedOutput.map(result -> result
                                        .resolveOutputsForInputArtifact(inputArtifact)).mapFailure(
                                        failure -> new TransformException(
                                                String.format("Execution failed for %s.",
                                                        execution.getDisplayName()), failure));
                            }
                        });
    }
    ...
    private ProjectInternal determineProducerProject(TransformationSubject subject) {
        ...
        if (componentIdentifier instanceof ProjectComponentIdentifier) {
            return projectStateRegistry.stateFor((ProjectComponentIdentifier) componentIdentifier)
                    .getMutableModel();
        } else {
            return null;
        }
    ...
    private <T> T fireTransformListeners(Transformer transformer,
                                         TransformationSubject subject,
    ...
    private static class ImmutableTransformerExecution extends AbstractTransformerExecution {
        ...
        public ImmutableTransformerExecution(Transformer transformer,
                                             File inputArtifact,
                                             ArtifactTransformDependencies dependencies,
                                             BuildOperationExecutor buildOperationExecutor,
                                             FileCollectionFactory fileCollectionFactory,
                                             ...
                                             FileSystemAccess fileSystemAccess,
                                             TransformationWorkspaceServices workspaceServices) {
            super(transformer, inputArtifact, dependencies, buildOperationExecutor,
                    fileCollectionFactory, inputFingerprinter, workspaceServices);
            this.fileSystemAccess = fileSystemAccess;
        }
        ...
        public void visitIdentityInputs(InputVisitor visitor) {
            ...
            // This is a performance hack. We could use the regular fingerprint of the input
            // artifact, but that takes longer than
            // capturing the normalized path and the snapshot of the raw contents, so we are
            // using these to determine the identity
            ...
            visitor.visitInputProperty(INPUT_ARTIFACT_PATH_PROPERTY_NAME, () -> {
                FileCollectionFingerprinter inputArtifactFingerprinter =
                        inputFingerprinter.getFingerprinterRegistry().getFingerprinter(
                                DefaultFileNormalizationSpec
                                        .from(transformer.getInputArtifactNormalizer(),
                                                transformer.getInputArtifactDirectorySensitivity(),
                                                transformer
                                                        .getInputArtifactLineEndingNormalization()));
                return inputArtifactFingerprinter.normalizePath(inputArtifactSnapshot);
            });
            visitor.visitInputProperty(INPUT_ARTIFACT_SNAPSHOT_PROPERTY_NAME,
    ...
    private static class MutableTransformerExecution extends AbstractTransformerExecution {
        public MutableTransformerExecution(Transformer transformer,
                                           File inputArtifact,
                                           ArtifactTransformDependencies dependencies,
                                           BuildOperationExecutor buildOperationExecutor,
                                           FileCollectionFactory fileCollectionFactory,
        ...
        public Identity identify(Map<String, ValueSnapshot> identityInputs,
                                 Map<String, CurrentFileCollectionFingerprint> identityFileInputs) {
            return new MutableTransformationWorkspaceIdentity(inputArtifact.getAbsolutePath(),
                    identityInputs.get(SECONDARY_INPUTS_HASH_PROPERTY_NAME),
                    identityFileInputs.get(DEPENDENCIES_PROPERTY_NAME).getHash());
        }
    }
    ...
    private abstract static class AbstractTransformerExecution implements UnitOfWork {
        ...
        public AbstractTransformerExecution(Transformer transformer,
                                            File inputArtifact,
                                            ArtifactTransformDependencies dependencies,
        ...
        public WorkOutput execute(ExecutionRequest executionRequest) {
            TransformationResult result =
                    buildOperationExecutor.call(new CallableBuildOperation<TransformationResult>() {
                        @Override
                        public TransformationResult call(BuildOperationContext context) {
                            File workspace = executionRequest.getWorkspace();
                            InputChangesInternal inputChanges =
                                    executionRequest.getInputChanges().orElse(null);
                            ImmutableList<File> result = transformer
                                    .transform(inputArtifactProvider, getOutputDir(workspace),
                                            dependencies, inputChanges);
                            TransformationResultSerializer resultSerializer =
                                    new TransformationResultSerializer(inputArtifact,
                                            getOutputDir(workspace));
                        ...
                        public BuildOperationDescriptor.Builder description() {
                            String displayName =
                                    transformer.getDisplayName() + " " + inputArtifact.getName();
        ...
        public Object loadRestoredOutput(File workspace) {
            TransformationResultSerializer resultSerializer =
                    new TransformationResultSerializer(inputArtifact, getOutputDir(workspace));
        ...
        public InputChangeTrackingStrategy getInputChangeTrackingStrategy() {
            return transformer
                    .requiresInputChanges() ? InputChangeTrackingStrategy.INCREMENTAL_PARAMETERS
                    : InputChangeTrackingStrategy.NONE;
        }
        ...
        public void visitIdentityInputs(InputVisitor visitor) {
            ...
            visitor.visitInputFileProperty(DEPENDENCIES_PROPERTY_NAME, NON_INCREMENTAL,
                    new FileValueSupplier(dependencies,
                            ...
                            () -> dependencies.getFiles().orElse(fileCollectionFactory.empty())));
        }
        ...
        public void visitRegularInputs(InputVisitor visitor) {
            visitor.visitInputFileProperty(INPUT_ARTIFACT_PROPERTY_NAME, INCREMENTAL,
                    new FileValueSupplier(inputArtifactProvider,
        ...
        public void visitOutputs(File workspace, OutputVisitor visitor) {
            ...
            visitor.visitOutputProperty(OUTPUT_DIRECTORY_PROPERTY_NAME, DIRECTORY, outputDir,
                    fileCollectionFactory.fixed(outputDir));
            visitor.visitOutputProperty(RESULTS_FILE_PROPERTY_NAME, FILE, resultsFile,
                    fileCollectionFactory.fixed(resultsFile));
        }

        @Override
        public Optional<CachingDisabledReason> shouldDisableCaching(@Nullable OverlappingOutputs detectedOverlappingOutputs) {
            return transformer.isCacheable() ? Optional.empty() : Optional.of(NOT_CACHEABLE);
        }
    ...
    private static class ImmutableTransformationWorkspaceIdentity implements UnitOfWork.Identity {
        ...
        public ImmutableTransformationWorkspaceIdentity(ValueSnapshot inputArtifactPath,
                                                        ValueSnapshot inputArtifactSnapshot,
                                                        ValueSnapshot secondaryInputSnapshot,
        ...
        public String getUniqueId() {
            ...
            Hashes.putHash(hasher, dependenciesHash);
        ...
        public boolean equals(Object o) {
            ...
            ImmutableTransformationWorkspaceIdentity that =
                    (ImmutableTransformationWorkspaceIdentity) o;
    ...
    public static class MutableTransformationWorkspaceIdentity implements UnitOfWork.Identity {
        ...
        public MutableTransformationWorkspaceIdentity(String inputArtifactAbsolutePath,
        ...
        public String getUniqueId() {
            Hasher hasher = Hashes.newHasher();
            hasher.putString(inputArtifactAbsolutePath, StandardCharsets.UTF_8);
        ...
        public boolean equals(Object o) {
            ...
            MutableTransformationWorkspaceIdentity that =
                    (MutableTransformationWorkspaceIdentity) o;

...
import com.tyron.builder.internal.snapshot.impl.GradleSerializedValueSnapshot;
import com.tyron.builder.internal.snapshot.impl.ImplementationSnapshot;
import com.tyron.builder.internal.snapshot.impl.JavaSerializedValueSnapshot;
...
public class SnapshotSerializer extends AbstractSerializer<ValueSnapshot> {
        ...
        switch (type) {
            ...
            case ATTRIBUTE:
                return new AttributeDefinitionSnapshot(
                        Attribute.of(decoder.readString(), Thread.currentThread().getContextClassLoader().loadClass(decoder.readString())),
                        classLoaderHasher
                );
    ...
    public void write(Encoder encoder, ValueSnapshot snapshot) throws Exception {
        if (snapshot == NullValueSnapshot.INSTANCE) {
            ...
            encoder.writeBinary(hashSnapshot.getValue().asBytes());

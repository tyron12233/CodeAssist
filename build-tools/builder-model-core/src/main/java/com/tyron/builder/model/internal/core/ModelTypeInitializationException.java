...
public class ModelTypeInitializationException extends BuildException {
        ...
        if (propertyContextOptional.isPresent()) {
            ...
            if (isManagedCollection(propertyType)) {
                ...
                CollectionSchema<?, ?> schema =
                        (CollectionSchema) schemaStore.getSchema(propertyType);
                s.append(String.format("A managed collection can not contain '%s's%n",
                        schema.getElementType()));
                appendManagedCollections(s, 1, constructibleTypes);
            } else if (isScalarCollection(propertyType,
                    propertyContext.isDeclaredAsHavingUnmanagedType())) {
    ...
    private static String explainScalarCollections(Iterable<? extends ModelType<?>> scalarTypes) {
        return String
                .format("A valid scalar collection takes the form of List<T> or Set<T> where 'T' " +
                        "is one of (%s)",
                        describe(scalarTypes));
    }

    private static String appendManagedCollections(StringBuilder s,
                                                   int pad,
                                                   Iterable<? extends ModelType<?>> constructibleTypes) {
        s.append(String.format(
                "A valid managed collection takes the form of ModelSet<T> or ModelMap<T> where " +
                "'T' is:%n        - %s",
    ...
    private static void maybeAppendConstructibles(StringBuilder s,
                                                  Iterable<? extends ModelType<?>> constructibleTypes,
                                                  int pad) {
        if (!Iterables.isEmpty(constructibleTypes)) {
            String padding = pad(pad);
            s.append(String.format("%n%s- or a type which Gradle is capable of constructing:",
                    padding));
    ...
    private static boolean isScalarCollection(ModelType<?> propertyType,

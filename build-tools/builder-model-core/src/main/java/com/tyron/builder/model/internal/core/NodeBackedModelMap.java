...
import com.tyron.builder.model.InvalidModelRuleException;
import com.tyron.builder.model.ModelMap;
import com.tyron.builder.model.ModelRuleBindingException;
import com.tyron.builder.model.RuleSource;
import com.tyron.builder.model.internal.core.rule.describe.ModelRuleDescriptor;
import com.tyron.builder.model.internal.inspect.ModelElementProjection;
import com.tyron.builder.model.internal.manage.instance.ManagedInstance;
import com.tyron.builder.model.internal.report.IncompatibleTypeReferenceReporter;
...
import static com.tyron.builder.model.internal.core.NodeInitializerContext.forExtensibleType;
...
public class NodeBackedModelMap<T> extends ModelMapGroovyView<T> implements ManagedInstance {
    ...
    private <E> void mutateChildren(ModelActionRole role, ModelType<E> filterType, String operation, Action<? super E> configAction) {
        ...
        modelNode.applyTo(allLinks(e -> elementFilter.withType(filterType).apply(e)), role, NoInputsModelAction.of(subject, descriptor, configAction));
    }

    private <E> void mutateChildren(ModelActionRole role, ModelType<E> filterType, DeferredModelAction configAction) {
        viewState.assertCanMutate();
        ModelReference<E> subject = ModelReference.of(filterType);
        modelNode.defineRulesFor(allLinks(e -> elementFilter.withType(filterType).apply(e)), role, new DeferredActionWrapper<E>(subject, role, configAction));
    }
    ...
    public Set<String> keySet() {
        viewState.assertCanReadChildren();
        return ImmutableSet.copyOf(modelNode.getLinkNames(elementFilter::apply));
    }

    @Override
    public int size() {
        viewState.assertCanReadChildren();
        return modelNode.getLinkCount(elementFilter::apply);
    }
    ...
    public <S> void withType(Class<S> type, Class<? extends RuleSource> rules) {
        viewState.assertCanMutate();
        modelNode.applyTo(allLinks(input -> elementFilter.withType(type).apply(input)), rules);
    }
    ...
    private interface PredicateSpec<T> {
        boolean apply(T t);
    }

    private static abstract class ElementFilter implements PredicateSpec<MutableModelNode>, Spec<ModelType<?>> {

...
/**
 * A {@code CancellationTokenSource} allows you to issue cancellation requests to one or more {@link com.tyron.builder.tooling.LongRunningOperation}
 * instances. To use a token source:
 *
 * <ul>
 *     <li>Create a token source using {@link GradleConnector#newCancellationTokenSource()}.</li>
 *     <li>Attach the token to one or more operations using {@link com.tyron.builder.tooling.LongRunningOperation#withCancellationToken(CancellationToken)}.
...
public interface CancellationTokenSource {
     ...
     * will notify its {@link com.tyron.builder.tooling.ResultHandler#onFailure(GradleConnectionException)}

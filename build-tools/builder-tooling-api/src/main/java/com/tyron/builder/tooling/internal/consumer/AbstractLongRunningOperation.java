...
import com.tyron.builder.tooling.LongRunningOperation;
import com.tyron.builder.tooling.ProgressListener;
...
public abstract class AbstractLongRunningOperation<T extends AbstractLongRunningOperation<T>> implements LongRunningOperation {
    ...
    public T addArguments(Iterable<String> arguments) {
        operationParamsBuilder.addArguments(ImmutableList.copyOf(Preconditions.checkNotNull(arguments)));
        return getThis();
    }
    ...
    public T addJvmArguments(Iterable<String> jvmArguments) {
        operationParamsBuilder.addJvmArguments(ImmutableList.copyOf(Preconditions.checkNotNull(jvmArguments)));
        return getThis();
    }
    ...
    @Override
    public T addProgressListener(com.tyron.builder.tooling.events.ProgressListener listener) {
        return addProgressListener(listener, EnumSet.allOf(OperationType.class));
    }

    @Override
    public T addProgressListener(com.tyron.builder.tooling.events.ProgressListener listener, OperationType... operationTypes) {
        return addProgressListener(listener, ImmutableSet.copyOf(operationTypes));
    }

    @Override
    public T addProgressListener(com.tyron.builder.tooling.events.ProgressListener listener, Set<OperationType> eventTypes) {

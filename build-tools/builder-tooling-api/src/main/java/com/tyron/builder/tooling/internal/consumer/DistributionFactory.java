...
import com.tyron.builder.util.internal.DistributionLocator;
import com.tyron.builder.util.internal.GFileUtils;
import com.tyron.builder.wrapper.GradleUserHomeLookup;
import com.tyron.builder.wrapper.SystemPropertiesHandler;
import com.tyron.builder.wrapper.WrapperConfiguration;
...
//        }
    ...
    public Distribution getDistribution(File gradleHomeDir) {
        return new InstalledDistribution(gradleHomeDir,
...
//        return new ZippedDistribution(configuration, clock);
    ...
    private static class ZippedDistribution implements Distribution {
        ...
        public ClassPath getToolingImplementationClasspath(ProgressLoggerFactory progressLoggerFactory,
            ...
            if (installedDistribution == null) {
                final DistributionInstaller installer =
                        new DistributionInstaller(progressLoggerFactory, progressListener, clock);
                File installDir;
                try {
                    ...
                    installDir = installer.install(determineRealUserHomeDir(connectionParameters),
                            determineRootDir(connectionParameters), wrapperConfiguration,
                            determineSystemProperties(connectionParameters));
                } catch (CancellationException e) {
                    throw new BuildCancelledException(String.format(
                    ...
                    throw new GradleConnectionException(
                            String.format("Could not install Gradle distribution from '%s'.",
                                    wrapperConfiguration.getDistribution()), e);
                }
            ...
            return installedDistribution
                    .getToolingImplementationClasspath(progressLoggerFactory, progressListener,
                            connectionParameters, cancellationToken);
        }

        private Map<String, String> determineSystemProperties(ConnectionParameters connectionParameters) {
            Map<String, String> systemProperties = new HashMap<String, String>();
            for (Map.Entry<Object, Object> entry : System.getProperties().entrySet()) {
                systemProperties.put(entry.getKey().toString(),
        ...
        private File determineRootDir(ConnectionParameters connectionParameters) {
            return new BuildLayoutFactory().getLayoutFor(connectionParameters.getProjectDir(),
                    connectionParameters.isSearchUpwards() != null ? connectionParameters
                            .isSearchUpwards() : true).getRootDirectory();
        }
    ...
    private static class InstalledDistribution implements Distribution {
        ...
        public ClassPath getToolingImplementationClasspath(ProgressLoggerFactory progressLoggerFactory,
            ...
            File[] files = libDir.listFiles(
                    file -> GFileUtils.hasExtensionIgnoresCase(file.getName(), ".jar"));

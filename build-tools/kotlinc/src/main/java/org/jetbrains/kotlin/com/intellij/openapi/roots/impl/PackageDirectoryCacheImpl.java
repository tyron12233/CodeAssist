...
import org.jetbrains.kotlin.com.intellij.openapi.roots.PackageDirectoryCache;
...
public final class PackageDirectoryCacheImpl implements PackageDirectoryCache  {
  ...
  private final Set<String> myNonExistentPackages = Collections.synchronizedSet(new HashSet<>());
  ...
  private @Nullable PackageInfo getPackageInfo(@NotNull String packageName) {
    ...
    if (info == null && !myNonExistentPackages.contains(packageName)) {
      if (packageName.length() > Registry.intValue("java.max.package.name.length") || Strings.containsAnyChar(packageName, ";[/")) {
        return null;
      }
  ...
  private final class PackageInfo {
    ...
    PackageInfo(@NotNull String qname, @NotNull List<? extends VirtualFile> packageDirectories) {
      ...
      mySubPackages = new VolatileNotNullLazyValue<MultiMap<String, VirtualFile>>() {
        @Override

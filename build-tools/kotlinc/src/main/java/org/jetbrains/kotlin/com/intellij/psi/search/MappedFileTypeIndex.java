...
import org.jetbrains.kotlin.com.intellij.util.indexing.containers.BitSetAsRAIntContainer;
import org.jetbrains.kotlin.com.intellij.util.indexing.containers.IntHashSetAsRAIntContainer;
...
import org.jetbrains.kotlin.com.intellij.util.indexing.containers.UpgradableRandomAccessIntContainer;
import org.jetbrains.kotlin.com.intellij.util.indexing.impl.ValueContainerImpl;
import org.jetbrains.kotlin.com.intellij.util.io.UnInterruptibleFileChannel;
...
public final class MappedFileTypeIndex extends FileTypeIndexImplBase {
  ...
  private static class IndexDataController {
    ...
    public synchronized void setAssociation(int inputId, short data) throws StorageException {
      ...
      if (indexedData != 0) {
        RandomAccessIntContainer indexedSet = myInvertedIndex.get(indexedData);
        assert indexedSet != null;
        indexedSet.remove(inputId);
      }
      myForwardIndex.set(inputId, data);
      if (data != 0) {
        myInvertedIndex.computeIfAbsent((int) data, __ -> createContainerForInvertedIndex()).add(inputId);
      }
  ...
  private static RandomAccessIntContainer createContainerForInvertedIndex() {
    return new UpgradableRandomAccessIntContainer<>(
      ...
      () -> {
        return new IntHashSetAsRAIntContainer(INVERTED_INDEX_SIZE_THRESHOLD, 0.75f);
      },
      (container) -> {
  ...
  private static @NotNull MappedFileTypeIndex.IndexDataController loadIndexToMemory(@NotNull Path forwardIndexStorageFile,
    ...
    forwardIndex.processEntries((inputId, data) -> {
      if (data != 0) {
        invertedIndex.computeIfAbsent((int) data, __ -> createContainerForInvertedIndex()).add(inputId);
      }
    });

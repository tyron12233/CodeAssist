...
public final class VfsEventsMerger {
  ...
  private final ConcurrentIntObjectHashMap<ChangeInfo> myChangeInfos =
          ((ConcurrentIntObjectHashMap) ConcurrentCollectionFactory.createConcurrentIntObjectMap());
  ...
  static class ChangeInfo {
    ...
    public String toString() {
      ...
      builder.append("file: ").append(file.getPath()).append("; ")
        .append("operation: ");
        if ((eventMask & FILE_TRANSIENT_STATE_CHANGED) != 0) {
            builder.append("TRANSIENT_STATE_CHANGE ");
        }
        if ((eventMask & FILE_CONTENT_CHANGED) != 0) {
            builder.append("UPDATE ");
        }
        if ((eventMask & FILE_REMOVED) != 0) {
            builder.append("REMOVE ");
        }
        if ((eventMask & FILE_ADDED) != 0) {
            builder.append("ADD ");
        }
...
//    private final RollingFileHandler myAppender;
    ...
    MyLoggerFactory() throws IOException {
      Path indexingDiagnosticDir = Paths.get(PathManager.getHomePath()).resolve("indexing-diagnostic");

...
final class FilePageCache {
  ...
  FilePageCache(final long cacheCapacityBytes) {
    ...
    pagesByPageId = new LongLinkedHashMap<DirectBufferWrapper>(10, 0.75f, /*access order: */ true) {
      ...
      @Override
      public DirectBufferWrapper put(long key, @NonNull DirectBufferWrapper wrapper) {
   ...
   */
  long registerPagedFileStorage(@NonNull PagedFileStorage storage) {
    synchronized (storageById) {
  ...
  @NonNull
  private Map<Long, DirectBufferWrapper> getBuffersForOwner(@NonNull PagedFileStorage storage) {

...
public final class IntToIntBtree {
  ...
  public IntToIntBtree(int pageSize,
                       @NonNull Path file,
                       @NonNull StorageLockContext storageLockContext,
    ...
    storage = new ResizeableMappedFile(file,
            pageSize,
            storageLockContext,
  ...
  public void persistVars(@NonNull BtreeDataStorage storage, boolean toDisk) throws IOException {
  ...
  /**
   * Lookup given key in BTree, and return true if key is exist in the Tree (associated value is
   * returned
   * in result[0]), false if there is no such key in the Tree (result is untouched then)
   *
   * @return true if key was found, false otherwise
   */
  public boolean get(int key, int[] result) throws IOException {
    ...
    if (root.address == UNDEFINED_ADDRESS) {
      return false;
    }
  ...
  @NonNull
  BTreeStatistics getStatistics() throws IOException {
    int leafPages = height == 3 ? pagesCount - (1 + root.getNodeView().getChildrenCount() + 1) :
            height ==
            2 ? pagesCount - 1 : 1;
    return new BTreeStatistics(pagesCount,
            count,
            height,
            movedMembersCount,
            leafPages,
  ...
  private interface NodeOp<T> {
    T operate(@NonNull DirectBufferWrapper buffer) throws IOException;
  }
  ...
  private class BtreeIndexNodeView implements Closeable {
    ...
    boolean processMappings(KeyValueProcessor processor) throws IOException {
      ...
      if (isHashedLeaf()) {
        ...
        for (int i = 0; i < hashPageCapacity; ++i) {
          ...
          try {
            result = ((NodeOp<Boolean>) b -> {
              ...
              if (key != HASH_FREE) {
                if (!processor.process(key, b.getInt(finalOffset))) {
                  return false;
                }
              }
              return true;
        ...
        for (int i = 0; i < childrenCount; ++i) {
          if (!processor.process(keyAt(i), addressAt(i))) {
            return false;
          }
        }
      }
        ...
        for (int i = 0; i < hashPageCapacity; ++i) {
          ...
          try {
            ...
            if (key != HASH_FREE) {
              ...
              if (keyNumber[0] == keys.length) {
                throw new CorruptedException(storage.getPagedFileStorage().getFile());
              }
      ...
      try {
        ...
        try (BtreeIndexNodeView newIndexNode = new BtreeIndexNodeView(nextPage(), true, null)) {
          ...
          if (indexLeaf && hashedLeaf) {
            ...
            ByteBuffer buffer =
                    getBytes(indexToOffset(maxIndex), recordCountInNewNode * INTERIOR_SIZE);
            ...
            if (indexLeaf) {
              ...
              --maxIndex;
              medianKey = keyAt(maxIndex);     // key count is odd (since children count is even)
              // and middle key goes to parent
            }
    ...
    private void insert(int valueHC, int newValueId) throws IOException {
      ...
      if (doSanityCheck) {
        assert recordCount < getMaxChildrenCount();
      }
      ...
      if (doSanityCheck) {
        if (index > 0) {
          assert keyAt(index - 1) < keyAt(index);
        }
        if (index < recordCount) {
          assert keyAt(index) < keyAt(index + 1);
        }
      }
    }
  ...
  public boolean processMappings(@NonNull KeyValueProcessor processor) throws IOException {
    ...
    if (hasZeroKey) {
      if (!processor.process(0, zeroKeyValue)) {
        return false;
      }
    }
    ...
    if (childrenAddresses.length > 0) {
      try (BtreeIndexNodeView child = new BtreeIndexNodeView(true)) {
        for (int childrenAddress : childrenAddresses) {
          ...
          if (!processLeafPages(child, processor)) {
            return false;
          }
        }
      }

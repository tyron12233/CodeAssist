...
import static org.jetbrains.kotlin.com.intellij.util.SystemProperties.getBooleanProperty;
import static org.jetbrains.kotlin.com.intellij.util.SystemProperties.getFloatProperty;
import static org.jetbrains.kotlin.com.intellij.util.SystemProperties.getIntProperty;
import static org.jetbrains.kotlin.com.intellij.util.io.IOUtil.MiB;
...
public final class PageCacheUtils {
  ...
  private static long estimateTotalCacheCapacityLimit(final long maxDirectMemoryToUseBytes) {
    ...
    final int defaultCacheCapacityMb = System.getProperty("ro.product.cpu.abilist", "64").contains("64") ? 500 : 200;


    final int cacheCapacityMb = getIntProperty("idea.max.paged.storage.cache", defaultCacheCapacityMb);
    final long cacheCapacityBytes = cacheCapacityMb * (long)(1024 * 1024);

    final int minCacheCapacityMb = 100;
    final long minCacheCapacityBytes = Math.min(minCacheCapacityMb * (1024 * 1024), maxDirectMemoryToUseBytes);
  ...
  static final int ALLOCATOR_SIZE = (int)Math.min(
    100 * MiB,
    Math.max(0, MAX_DIRECT_MEMORY_TO_USE_BYTES - FILE_PAGE_CACHES_TOTAL_CAPACITY_BYTES - 300 * (1024 * 1024))
  );

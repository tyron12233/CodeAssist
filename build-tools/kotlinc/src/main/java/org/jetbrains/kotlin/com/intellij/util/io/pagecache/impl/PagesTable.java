...
public class PagesTable {
  ...
  public PageImpl lookupOrCreate(final int pageIndex,
                                 final @NonNull IntFunction<PageImpl> uninitializedPageFactory,
                                 final @NonNull PageContentLoader pageContentLoader) throws IOException {
  ...
  private PageImpl insertNewPage(final int pageIndex,
    ...
    final Ref<Integer> insertionIndexRef = new Ref<>();
   ...
   */
  //@GuardedBy(pagesLock.writeLock)
  private static int rehashWithoutTombstones(final @NonNull AtomicReferenceArray<PageImpl> sourcePages,
                                             final @NonNull AtomicReferenceArray<PageImpl> targetPages) {
    final Ref<Integer> insertionIndexRef = new Ref<>();
    int pagesCopied = 0;
  ...
  @Nullable
  private static PageImpl findPageOrInsertionIndex(final @NonNull AtomicReferenceArray<PageImpl> pages,
                                                   final int pageIndex,
                                                   final @Nullable Ref<Integer> insertionIndexRef) {
  ...
  public String probeLengthsHistogram() {
    ...
    final int sum = histo.values().stream().mapToInt(Integer::intValue).sum();
    return histo.keySet().stream().mapToInt(Integer::intValue)
      .sorted()
      .mapToObj(len -> String.format(Locale.ENGLISH,"%3d: %4d (%4.1f%%)", len, histo.get(len), histo.get(len) * 100.0 / sum))
      .collect(joining("\n"));
  }
  ...
  public Int2IntMap collectProbeLengthsHistogram() {
    ...
    for (int i = 0; i < pages.length(); i++) {
      ...
      if (page != null) {
        ...
        histo.merge(probingLength, 1, Integer::sum);
      }
    }
    return histo;
  }

  private static int probingSequenceLengthFor(final @NonNull AtomicReferenceArray<PageImpl> pages,
                                              final int pageIndex) {

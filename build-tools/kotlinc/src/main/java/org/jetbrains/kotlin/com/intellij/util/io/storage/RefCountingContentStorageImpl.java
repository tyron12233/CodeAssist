...
public class RefCountingContentStorageImpl extends AbstractStorage implements RefCountingContentStorage {
    ...
    private final IntObjectMap<RecordData> myCurrentRecords =
    ...
    protected void doDeleteRecord(int record) throws IOException {
        if (myUseContentHashes) {
            throw new UnsupportedEncodingException(
                    "Records can't be released completely with enabled content hashes support");
        }
    ...
    private BufferExposingByteArrayOutputStream internalReadStream(int record) throws IOException {
        ...
        try (InflaterInputStream in = new CustomInflaterInputStream(result)) {
            final BufferExposingByteArrayOutputStream outputStream =
    ...
    private void doRecordSanityCheck(int record, byte[] result) {
        ...
        if (savedData.compressedSize != result.length ||
            ...
            String msg = "expected compressed len = " +
                         savedData.compressedSize +
                         ", but actual len = " +
                         result.length +
                         ", \n" +
                         " expected content hash = " +
                         savedData.compressedHash +
                         ", but actual hash = " +
                         currentHash;
    ...
    private static class CustomInflaterInputStream extends InflaterInputStream {
        CustomInflaterInputStream(byte[] compressedData) {
            ...
            // force to directly use compressed data, this ensures less round trips with native
          // extraction code and copy streams
    ...
    @Override
    public void writeBytes(final int record,
                           final ByteArraySequence bytes,
    ...
    private void zipAndWrite(ByteArraySequence bytes,
                             int record,
                             boolean fixedSize) throws IOException {
        ...
        withWriteLock(() -> {
            ...
            if (IndexDebugProperties.IS_UNIT_TEST_MODE) {
                myCurrentRecords.put(record,
                        new RecordData(compressedBytes.getLength(), compressedBytes.hashCode()));
            }
            myPendingWriteRequestsSize -= bytes.getLength();
    ...
    @Override
    protected RefCountingRecordsTable createRecordsTable(@NotNull StorageLockContext storageLockContext,

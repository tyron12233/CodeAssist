...
public class XmlPrettyPrinter {
    ...
    private void printComment(int depth, Node node) {
        ...
        if (!multiLine) {
            ...
            mOut.append(XmlUtils.XML_COMMENT_BEGIN).append(' ');
            mOut.append(trimmed);
            mOut.append(' ').append(XmlUtils.XML_COMMENT_END);
            mOut.append(mLineSeparator);
        } else {
            ...
            if (start >= end) {
                ...
                mOut.append(XmlUtils.XML_COMMENT_BEGIN).append(' ').append(XmlUtils.XML_COMMENT_END);
                mOut.append(mLineSeparator);
                return;
            }
            ...
            if (multiLine) {
                indent(depth);
                mOut.append(XmlUtils.XML_COMMENT_BEGIN);
                mOut.append(mLineSeparator);
                ...
                if (!startsWithNewline) {
                    ...
                    if (previous != null && previous.getNodeType() == Node.TEXT_NODE) {
                        ...
                        int indentation = XmlUtils.XML_COMMENT_BEGIN.length();
                ...
                mOut.append(mLineSeparator);
                indent(depth);
                mOut.append(XmlUtils.XML_COMMENT_END);
                mOut.append(mLineSeparator);
            } else {
                mOut.append(XmlUtils.XML_COMMENT_BEGIN).append(' ');
                mOut.append(trimmed);
    ...
    private void printOpenElementTag(int depth, Node node) {
        ...
        if (attributeCount > 0) {
            ...
            if (singleLine || (depth == 0 && SdkConstants.XMLNS.equals(attributeList.get(0).getPrefix()))) {
                mOut.append(' ');
    ...
    private boolean isMarkupElement(Element element) {
        ...
        while (curr != null) {
            if (SdkConstants.TAG_STRING.equals(curr.getNodeName())) {
                return true;
            }
    ...
    private boolean isSingleLineTag(Element element) {
        ...
        return (tag.equals(SdkConstants.TAG_ITEM) && mStyle == XmlFormatStyle.RESOURCE)
                || tag.equals(SdkConstants.TAG_STRING)
                || tag.equals(SdkConstants.TAG_DIMEN)
                || tag.equals(SdkConstants.TAG_COLOR);
    }
    ...
    protected boolean isEmptyTag(Element element) {
        ...
        String tag = element.getTagName();
        if (SdkConstants.TAG_STRING.equals(tag)) {
            return false;
        }
    ...
    private static void formatFile(@NotNull XmlFormatPreferences prefs, File file,
        ...
        } else if (file.isFile() && SdkUtils.endsWithIgnoreCase(file.getName(), SdkConstants.DOT_XML)) {

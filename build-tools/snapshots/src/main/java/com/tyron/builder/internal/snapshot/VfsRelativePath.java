...
public class VfsRelativePath {
    ...
    private static String normalizeRoot(String absolutePath) {
        ...
        return PathUtil.isFileSeparator(absolutePath.charAt(absolutePath.length() - 1))
    ...
    private static int determineOffset(String absolutePath) {
        for (int i = 0; i < absolutePath.length(); i++) {
            if (!PathUtil.isFileSeparator(absolutePath.charAt(i))) {
                return i;
            }
        }
    ...
    public int lengthOfCommonPrefix(String relativePath, CaseSensitivity caseSensitivity) {
        ...
        for (; pos < maxPos; pos++) {
            ...
            if (!PathUtil.equalChars(charInPath1, charInPath2, caseSensitivity)) {
                break;
            }
            if (PathUtil.isFileSeparator(charInPath1)) {
                lastSeparator = pos;
            }
        }
        if (pos == maxPos) {
            ...
            if (pos < absolutePath.length() - offset && PathUtil
                    .isFileSeparator(absolutePath.charAt(pos + offset))) {
                return pos;
            }
        }
    ...
    public int compareToFirstSegment(String relativePath, CaseSensitivity caseSensitivity) {
        ...
        for (int pos = 0; pos < maxPos; pos++) {
            ...
            int comparedChars = PathUtil.compareCharsIgnoringCase(charInPath1, charInPath2);
            if (comparedChars != 0) {
        ...
        if (absolutePath.length() - offset > relativePath.length()) {
            return PathUtil.isFileSeparator(absolutePath.charAt(maxPos + offset)) ? accumulatedValue : 1;
        }
    ...
    private static int computeCombinedCompare(int previousCombinedValue, char charInPath1, char charInPath2, boolean caseSensitive) {
        ...
        return previousCombinedValue == 0
                ? PathUtil.compareChars(charInPath1, charInPath2)

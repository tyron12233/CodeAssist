...
import com.tyron.psi.completions.lang.java.JavaPsiClassReferenceElement;
import com.tyron.psi.editor.Editor;
import com.tyron.psi.lookup.DefaultLookupItemRenderer;
...
public final class PsiTypeLookupItem extends LookupItem implements TypedLookupItem {
    private static final InsertHandler<PsiTypeLookupItem> DEFAULT_IMPORT_FIXER = (context, item) -> {
    ...
    public void handleInsert(@NotNull InsertionContext context) {
        ...
        if (position != null) {
            ...
            DocumentUtils.insertString(context.getDocument(), genericsStart, JavaCompletionUtil.escapeXmlIfNeeded(context, calcGenerics(position, context)));
        ...
        if (!braces.isEmpty()) {
            if (myAddArrayInitializer) {
                DocumentUtils.insertString(context.getDocument(), targetOffset, braces + "{}");

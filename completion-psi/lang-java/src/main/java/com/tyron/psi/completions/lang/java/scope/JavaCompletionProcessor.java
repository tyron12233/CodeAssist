package com.tyron.psi.completions.lang.java.scope;
...
//                Registry.is("ide.java.completion.suggest.static.after.instance");
    ...
    private void addSealedHierarchy(@NotNull ResolveState state, @NotNull PsiClass psiClass) {
        final Collection<PsiClass> sealedInheritors = new ArrayList<>();//SealedUtils.findSameFileInheritorsClasses(psiClass);
...
//        });
    ...
    private boolean isAccessibleForResolve(@Nullable PsiElement element) {
        if (element instanceof PsiMember) {
            ...
            if (!myQualified) {
                accessObjectClasses = Collections.singleton(null);
            } else {
                if (myQualifierType instanceof PsiIntersectionType)
                    accessObjectClasses = ContainerUtil.map2Set(Arrays.asList(((PsiIntersectionType) myQualifierType).getConjuncts()), new Function<PsiType, PsiClass>() {
                        @Override
                        public PsiClass fun(PsiType psiType) {
                            return PsiUtil.resolveClassInClassTypeOnly(psiType);
                        }
                    });
                else
                    accessObjectClasses = Collections.singleton(PsiUtil.resolveClassInClassTypeOnly(myQualifierType));
            }

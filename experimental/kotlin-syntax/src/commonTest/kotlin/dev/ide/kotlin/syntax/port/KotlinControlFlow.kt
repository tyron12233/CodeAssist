// PORT PROBE — a VERBATIM copy of :lang-kotlin's resolve/KotlinControlFlow.kt with its IMPORTS rewritten to
// this module's facade, and nothing else changed.
//
// It exists to answer one question honestly: would the editor backend's PSI-walking code compile against the
// Kt* facade over the vendored tree? 443 lines, 26 PSI imports, no other coupling, so what fails to compile is
// a gap in the facade rather than a missing dependency. Every compile error here is a real finding.
//
// It is NOT a second implementation and must never be edited to make it pass: the only legitimate fixes are in
// the facade. Re-copy it from :lang-kotlin when that file changes.
...
internal class KotlinControlFlow(private val resolver: KotlinResolver) {
    ...
    private fun collectUninitLocals(root: KtElement, out: MutableSet<KtProperty>) {
        fun rec(p: KtElement) {
    ...
    private fun markLoopOrClosureAssigned(
        root: KtElement, tracked: Set<KtProperty>, complex: MutableSet<KtProperty>
    ) {
        fun rec(p: KtElement, inLoopOrClosure: Boolean) {
            val here =

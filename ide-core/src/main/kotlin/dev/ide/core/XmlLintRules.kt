...
object XmlLintRules {
    ...
    /** `xmlns:android` is missing although `android:*` attributes are used. [insertAt] is where to splice it. */
    data class MissingNamespace(val range: TextRange, val insertAt: Int, val uri: String = ANDROID_NS)
    ...
    fun missingNamespace(parsed: ParsedFile): MissingNamespace? {
        val tags = allTags(parsed)
        val root = tags.firstOrNull() ?: return null
        val usesAndroid = tags.any { t -> t.attributes.any { it.name?.startsWith("android:") == true } }
        if (!usesAndroid || root.attributes.any { it.name == "xmlns:android" }) return null
        val at = root.startOffset + 1 + (root.name?.length ?: 0)
        return MissingNamespace(TextRange(root.startOffset, at), at)
    }

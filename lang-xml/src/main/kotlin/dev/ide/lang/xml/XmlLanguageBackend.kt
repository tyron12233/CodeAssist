 ...
 * completion only: XML is not compiled here — `res/` is processed by the Android build system (aapt2),
 * so [createCompiler] is null. Error recovery is inherent to the tolerant parser; bindings/incremental
 * are not advertised (no symbol model yet; full reparse per change).
 */
class XmlLanguageBackend : LanguageBackend {
    ...
    override fun createCompiler(ctx: CompilationContext): SourceCompiler? = null

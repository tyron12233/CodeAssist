 ...
 *
 * Change detection is by TEXT, not PSI identity: the incremental reparse keeps the SAME composite KtElement
...
internal object IncrementalDecls {
     ...
     */
    private fun collectBodyCuts(psi: KtElement, base: Int, out: MutableList<IntArray>) {
        val body = when (psi) {
    ...
    fun providesOperator(d: KtDeclaration): Boolean = when (d) {
        is KtNamedFunction -> d.hasModifier(KtTokens.OPERATOR_KEYWORD)
        is KtClassOrObject -> d.declarations.any {
            it is KtNamedFunction && it.hasModifier(KtTokens.OPERATOR_KEYWORD)
        }

 ...
 *
 * Public, because it is the entry point a host without a `CompilationContext` analyzes through. The desktop
 * and Android hosts reach it via `KotlinSourceAnalyzer`, which binds this module to a build; a host that has
 * no build (iOS) holds a [KotlinSymbolService] and one of these directly, which is all that adapter does for
 * diagnostics anyway.
 */
class IncrementalSemanticAnalysis(
    ...
    /**
     * Whether the classpath can answer well enough to call a name UNRESOLVED.
     *
     * Defaults to the symbol service's own answer, which is "yes unless an index is still building". That
     * default assumes what is true on the JVM: with no index wired, the live `ClasspathReader` IS the
     * classpath and answers everything the index would.
     *
     * A host where that does not hold has to say so. On iOS the reader resolves a type by FQN and its
     * members, but a NAME lookup (`println` -> the callables that could be it) is an index operation and
     * there is no index, so every stdlib call would be reported unresolved — the app's own starter file
     * opened with an error in it. Withholding is the same thing the JVM does while its index builds: a
     * missing diagnostic, never a wrong one.
     */
    private val resolveReady: () -> Boolean = { service.classpathReady() },
) {

...
class KotlinCodeFolder(
     ...
     *  range is on record. */
    private fun walkFolds(root: KtElement, text: CharSequence, out: MutableList<FoldRegion>) {
        var seen = 0
        var tripped = false
                ...
                if (!tripped) {
                    ...
                    foldLog.warn(
                        "fold walk exceeded $MAX_FOLD_NODES nodes; bailing at " +
                            "${psi::class.simpleName} range=${psi.textRange} — folds may be incomplete"
                    )
                }
                return
            }
            // A comment is a TOKEN to this parser, and trivia, so it is matched by element type rather than by
            // class (there is no `PsiComment`/`KDoc` to test against) and the walk below has to descend the
            // trivia-INCLUDING children or it would never reach one.
            when {
                psi is KtClassBody -> braceBlock(out, text, psi.lBrace, psi.rBrace, FoldKind.CLASS_BODY)
                psi is KtBlockExpression -> braceBlock(out, text, psi.lBrace, psi.rBrace, FoldKind.FUNCTION_BODY)
                psi.elementType == KtTokens.DOC_COMMENT ->
                    addRegion(out, text, psi.textRange.startOffset, psi.textRange.endOffset, "/**...*/", FoldKind.COMMENT)
                psi.elementType == KtTokens.BLOCK_COMMENT ->
                    addRegion(out, text, psi.textRange.startOffset, psi.textRange.endOffset, "/*...*/", FoldKind.COMMENT)
            }
            for (c in psi.childrenWithTrivia) {
                if (tripped) break
                walk(c)
            }
        }
    ...
    private fun braceBlock(out: MutableList<FoldRegion>, text: CharSequence, lBrace: KtElement?, rBrace: KtElement?, kind: FoldKind) {

...
class KotlinFormatter : FormattingService {
    ...
    companion object {
        private val OPENERS: Set<SyntaxElementType> = setOf(KtTokens.LBRACE, KtTokens.LPAR, KtTokens.LBRACKET)
        private val CLOSERS: Set<SyntaxElementType> = setOf(KtTokens.RBRACE, KtTokens.RPAR, KtTokens.RBRACKET)

        /** Comment kinds, whose interior the formatter must leave exactly as the author wrote it. */
        private val COMMENT_TYPES: Set<SyntaxElementType> =
            setOf(KtTokens.BLOCK_COMMENT, KtTokens.DOC_COMMENT, KtTokens.EOL_COMMENT, KtTokens.SHEBANG_COMMENT)
        ...
        private fun scan(ktFile: KtFile, src: String): Scan {
            ...
            fun walk(e: KtElement) {
                // A comment is a TOKEN to this parser and has no class to test for, so the guard is on the
                // element type. It matters: this is what stops the formatter reflowing the inside of a
                // multi-line comment, and a class test that can no longer match turns that protection off
                // without failing anything.
                val protectedNode = e is KtStringTemplateExpression || e.elementType in COMMENT_TYPES
                if (protectedNode) {
                    val r = e.textRange
                    if (hasNewline(src, r.startOffset, r.endOffset)) protectedRanges += intArrayOf(r.startOffset, r.endOffset)
                    protectDepth++
                }
                // Trivia included: a comment is not in `children`, so the walk would never reach one to
                // protect it, and the token stream this builds would be missing them entirely.
                val kids = e.childrenWithTrivia
                if (kids.isEmpty()) {
                    if (protectDepth == 0) {
                        val r = e.textRange
                        if (r.endOffset > r.startOffset) {
                            val et = e.elementType
        ...
        private class Atom(val start: Int, val end: Int, val type: SyntaxElementType?)

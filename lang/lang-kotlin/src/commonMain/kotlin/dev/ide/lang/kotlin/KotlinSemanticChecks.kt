...
import dev.ide.kotlin.syntax.psi.KtProjectionKind
...
import dev.ide.kotlin.syntax.psi.KtTypeElement
...
import dev.ide.kotlin.syntax.psi.Variance
...
internal class KotlinSemanticChecks(private val service: KotlinSymbolService) {
    ...
    fun checkers(): List<KotlinChecker> = listOf(
        KotlinChecker(KtDeclaration::class) { psi ->
            KotlinPerf.span("sem.modifiers") { reportAll(modifierConflicts(psi as KtDeclaration)) }
        },
        KotlinChecker(KtNameReferenceExpression::class) { psi ->
        ...
        KotlinChecker(KtUserType::class) { psi ->
            psi as KtUserType
        ...
        KotlinChecker(KtProperty::class) { psi ->
            psi as KtProperty
        ...
        KotlinChecker(KtParameter::class) { psi ->
            psi as KtParameter
        ...
        KotlinChecker(KtNamedFunction::class) { psi ->
        ...
        KotlinChecker(KtWhenExpression::class) { psi ->
        ...
        KotlinChecker(KtBinaryExpressionWithTypeRHS::class) { psi ->
        ...
        KotlinChecker(KtIsExpression::class) { psi ->
            KotlinPerf.span("sem.isCheck") { if (resolveReady) report(uselessIsCheck(psi as KtIsExpression, resolver)) }
        },
        ...
        KotlinChecker(KtDestructuringDeclaration::class) { psi ->
        ...
        KotlinChecker(KtCallExpression::class) { psi ->
        ...
        KotlinChecker(KtPrefixExpression::class) { psi ->
            psi as KtPrefixExpression
            KotlinPerf.span("sem.valIncDec") { report(valIncDecReassignment(psi) ?: incrementTargetExpected(psi)) }
        },
        KotlinChecker(KtTryExpression::class) { psi ->
            KotlinPerf.span("sem.tryCatch") { if (resolveReady) reportAll(unreachableCatch(psi as KtTryExpression, resolver)) }
        },
        KotlinChecker(KtStringTemplateExpression::class) { psi ->
        ...
        KotlinChecker(KtParameterList::class) { psi ->
            KotlinPerf.span("sem.dup") { reportAll(duplicateParams(psi as KtParameterList)) }
        },
        KotlinChecker(KtBlockExpression::class) { psi ->
        ...
        KotlinChecker(KtClassBody::class) { psi ->
        ...
        KotlinChecker(KtClass::class) { psi ->
        ...
        KotlinChecker(KtClassOrObject::class) { psi ->
    ...
    private fun isSamConversion(call: KtCallExpression): Boolean {
        ...
        return arg is KtLambdaExpression ||
            arg is KtCallableReferenceExpression ||
    ...
    private fun classifierUsedAsValue(ref: KtNameReferenceExpression, resolver: KotlinResolver): Diagnostic? {
        ...
        if (ref.parent is KtInstanceExpressionWithLabel) return null // this/super
        if (hasAncestor(ref) { it is KtAnnotationEntry }) return null
        ...
        if (fp is KtDoubleColonExpression) return null               // `Foo::class` / `Foo::member`
    ...
    private fun functionUsedAsValue(ref: KtNameReferenceExpression, resolver: KotlinResolver): Diagnostic? {
        ...
        if (parent is KtCallableReferenceExpression) return null
        if (parent is KtDoubleColonExpression) return null
        ...
        val sameFile = ref.containingKtFile.declarations.filter {
            it is KtNamedDeclaration && it.name == name
        }
     ...
     *  scans, no resolution). */
    fun localDeclarationChecks(root: KtElement, out: MutableList<Diagnostic>) {
        fun rec(p: KtElement) {
    ...
    private fun localNameShadowing(prop: KtProperty): Diagnostic? {
        ...
        var node: KtElement? = prop.parent?.parent // start ABOVE the declaring block (an outer scope shadows)
        while (node != null) {
            when (node) {
    ...
    private fun shadowDiag(id: KtElement, name: String): Diagnostic {
        val r = id.textRange
    ...
    private fun assignmentInExpression(e: KtBinaryExpression): Diagnostic? {
        if (e.operationToken != KtTokens.EQ && e.operationToken !in ASSIGN_OPS) return null
        var child: KtElement = e
        var parent = e.parent
        while (parent is KtParenthesizedExpression) { child = parent; parent = parent.parent }
        val inExprContext = when (parent) {
            ...
            is KtValueArgument -> parent.getArgumentExpression() === child
    ...
    private fun unreachableCatch(t: KtTryExpression, resolver: KotlinResolver): List<Diagnostic> {
    ...
    private fun nearestLocalDecl(name: String, offset: Int, from: KtElement): KtElement? {
    ...
    private fun referenceCounts(root: KtElement): Map<String, Int> {
    ...
    private fun assignedInConstructor(cls: KtClassOrObject, name: String): Boolean {
        val bodies = ArrayList<KtElement>()
    ...
    private fun assignsName(root: KtElement, name: String): Boolean {
        var found = false
        fun rec(e: KtElement) {
            if (found) return
    ...
    private fun modifierConflicts(decl: KtDeclaration): List<Diagnostic> {
        ...
        fun flag(a: KtElement, b: KtElement, msg: String) {
            for (e in listOf(a, b)) {
                val r = e.textRange
    ...
    private fun constructorCallMismatch(call: KtCallExpression, resolver: KotlinResolver): Diagnostic? {
        ...
        if (!variadic && ctors.none { it.paramTypes.size == n }) {
            val arities = ctors.map { it.paramTypes.size }.distinct().sorted().joinToString("/")
            val r = call.valueArgumentList?.textRange ?: callee.textRange
            return Diagnostic(
                TextRange(r.startOffset, r.endOffset), Severity.ERROR,
    ...
    private fun sameFileConstructorMismatch(call: KtCallExpression, resolver: KotlinResolver): Diagnostic? {
        ...
        if (ctors.none { n in it.count { p -> !p.hasDefaultValue() }..it.size }) {
            val arities = ctors.flatMap { it.count { p -> !p.hasDefaultValue() }..it.size }.distinct().sorted().joinToString("/")
            val r = call.valueArgumentList?.textRange ?: callee.textRange
            return Diagnostic(
                TextRange(r.startOffset, r.endOffset), Severity.ERROR,
                "No constructor of '$name' takes $n argument(s) (expected $arities)", KotlinDiagnosticCodes.CONSTRUCTOR_ARGS,
    ...
    private fun sameFileClassNamed(call: KtCallExpression, name: String): KtClass? {
        ...
        fun collect(decls: List<KtDeclaration>) {
            for (d in decls) if (d is KtClassOrObject && d !is KtEnumEntry) {
                d.name?.let { byName.getOrPut(it) { d } }
    ...
    private fun isModeledWhenCondition(c: KtWhenCondition): Boolean = when (c) {
    ...
    private fun hasUnmodeledNullGuard(from: KtElement, name: String): Boolean {
    ...
    private fun unresolvedBareReference(expr: KtNameReferenceExpression, resolver: KotlinResolver): Diagnostic? {
        ...
        if (parent is KtInstanceExpressionWithLabel) return null
        if (inImportOrPackage(expr) || inTypeReference(expr)) return null
        val name = expr.getReferencedName()
        if (name.isEmpty()) return null
        if (name == "it" && hasAncestor(expr) { it is KtLambdaExpression }) return null
    ...
    private fun checkArgsAgainstBounds(args: List<KtTypeProjection>, bounds: List<KotlinType?>, resolver: KotlinResolver): List<Diagnostic> {
        val out = ArrayList<Diagnostic>()
    ...
    private fun inImportOrPackage(expr: KtElement): Boolean =
    ...
    private fun reportOptInUsage(markers: List<OptInMarker>, anchor: KtElement): Diagnostic? {
    ...
    private fun optedIn(useSite: KtElement, markerSimple: String): Boolean {
        var node: KtElement? = useSite
        while (node != null) {
            when (node) {
                is KtFile ->
    ...
    private fun annotationsOptIn(entries: List<KtAnnotationEntry>, markerSimple: String): Boolean {

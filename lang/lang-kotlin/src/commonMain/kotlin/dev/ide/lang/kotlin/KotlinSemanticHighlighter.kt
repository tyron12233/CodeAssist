...
class KotlinSemanticHighlighter(
    ...
    private fun collectInto(
        ...
        var seen = 0
        fun emit(
            range: dev.ide.kotlin.syntax.psi.TextRange?,
            kind: HighlightKind,
        ...
        fun walk(psi: KtElement) {
            ...
            when (psi) {
                ...
                is KtAnnotationEntry -> {
                    ...
                    emit(dev.ide.kotlin.syntax.psi.TextRange(at, at + 1), HighlightKind.ANNOTATION)
                }
    ...
    private fun classifyStringTemplate(
        ...
        emit: (dev.ide.kotlin.syntax.psi.TextRange?, HighlightKind, Set<HighlightModifier>) -> Unit
    ) {
    ...
    private fun topLevelPropertyInFile(name: String, from: KtElement): Boolean? =
        (from.containingFile as? KtFile)?.declarations
    ...
    private fun enclosingClassMemberProperty(name: String, from: KtElement): Boolean? {
    ...
    private fun localOrParamDecl(name: String, offset: Int, from: KtElement): KtElement? {
    ...
    private fun isTypeParameterInScope(from: KtElement, name: String): Boolean {

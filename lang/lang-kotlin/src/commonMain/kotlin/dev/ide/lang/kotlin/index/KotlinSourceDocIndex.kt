 ...
 * this runs the Kotlin lexer ([KotlinSyntax.lex]) and walks tokens rather than building a whole parse
...
object KotlinSourceDocIndex : IndexExtension<String, SourceDocValue> {
    ...
    private class Tok(val type: SyntaxElementType, val text: String)

    /** Lex into the significant tokens (whitespace + non-KDoc comments dropped; KDoc kept). */
    private fun lex(text: CharSequence): List<Tok> {
        // The vendored lexer hands back the whole token list rather than a cursor to advance, so this is an
        // indexed walk where it used to be a `while (advance())` loop.
        val tokens = KotlinSyntax.lex(text)
        val out = ArrayList<Tok>()
        for (i in 0 until tokens.tokenCount) {
            val t = tokens.getTokenType(i) ?: continue
            if (t != KtTokens.WHITE_SPACE && t != KtTokens.EOL_COMMENT &&
                t != KtTokens.BLOCK_COMMENT && t != KtTokens.SHEBANG_COMMENT
            ) out.add(Tok(t, text.substring(tokens.getTokenStart(i), tokens.getTokenEnd(i))))
        }
        return out
    }

...
fun KotlinResolver.composableContextAt(offset: Int): ComposableContext {
    ...
    val boundary = run {
        var n: KtElement? = elementAt(offset)
        while (n != null && n !is KtNamedFunction && n !is KtPropertyAccessor && n !is KtLambdaExpression) n =
            n.parent
        n
    }
...
private fun KtAnnotated.hasComposableAnnotation(): Boolean =
...
internal fun KotlinResolver.suspendContextWalk(offset: Int): SuspendContext {
    ...
    while (node != null) {
        when (node) {
            ...
            is KtPropertyAccessor -> return SuspendContext.NON_SUSPEND
            is KtLambdaExpression -> {
                val expected = expectedFunctionTypeFor(node)

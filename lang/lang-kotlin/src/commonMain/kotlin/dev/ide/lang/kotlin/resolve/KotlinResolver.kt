...
class KotlinResolverCaches {
    ...
    val composeCtx = HashMap<KtElement, ComposableContext>()
    val suspendCtx = HashMap<KtElement, SuspendContext>()
    ...
    val varNonNull = HashMap<KtElement, Set<KtElement>>()
...
class KotlinResolver(
    ...
    internal fun recordOverrideConsult(lambda: KtLambdaExpression, value: KotlinSymbolService.FunctionalShape?) {
        ...
        for (f in scoringDepFrames) if (lambda !in f.shadowed) f.deps.getOrPut(lambda) { value }
    }
    ...
    internal fun propagateDeps(deps: Map<KtLambdaExpression, KotlinSymbolService.FunctionalShape?>) {
        for (f in scoringDepFrames) for ((l, s) in deps) if (l !in f.shadowed) f.deps.getOrPut(l) { s }
    }
    ...
    internal val inferringReturnBodies = HashSet<KtElement>()
    ...
    internal fun elementAt(offset: Int): KtElement? {

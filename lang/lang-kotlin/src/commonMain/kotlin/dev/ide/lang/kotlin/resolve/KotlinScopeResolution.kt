        ...
        when (node) {
            ...
            is KtClassOrObject -> {
                val fqn = node.fqName?.asString()
                    ?: (node as? KtObjectDeclaration)?.let {
                        dev.ide.lang.kotlin.symbols.SourceIndexBuilder.localTypeFqn(
                            it
                        )
                    }
                    ?: (node as? KtClass)?.takeIf { it !is KtEnumEntry }
 ...
 */
fun KotlinResolver.callableReferenceTarget(e: KtCallableReferenceExpression): KotlinSymbol? {
    val name = e.callableReference?.getReferencedName().orEmpty()
    if (name.isEmpty()) return null
...
internal inline fun forEachLocalFunction(block: KtBlockExpression, action: (KtNamedFunction) -> Unit) {
    var child: KtElement? = block.firstChild
    while (child != null) {
...
internal fun KotlinResolver.localFunction(fn: KtNamedFunction): KotlinSymbol {
    ...
    return KotlinSymbol(
        ...
        isInline = fn.hasModifier(KtTokens.INLINE_KEYWORD),
...
internal fun KotlinResolver.accessorScopeLocals(offset: Int): List<KotlinSymbol> {
    val accessor = elementAt(offset)?.getParentOfType<KtPropertyAccessor>(strict = true)
        ?: return emptyList()
...
private fun classCtorParams(e: KtElement): List<KtParameter> =
    e.getParentOfType<KtClassOrObject>(strict = true)
...
fun KotlinResolver.isTypeParameterInScope(name: String, offset: Int): Boolean {
    var node: KtElement? = elementAt(offset)
    while (node != null) {
        if (node is KtTypeParameterListOwner && node.typeParameters.any { it.name == name }) return true
        node = node.parent
    }
    return false
}
...
fun KotlinResolver.typeParameterUpperBound(
    ...
    var node: KtElement? = elementAt(offset)
    while (node != null) {
        if (node is KtTypeParameterListOwner) {
        ...
        if (node is KtClassOrObject) {
            if (node.declarations.any {
                    (it is KtNamedFunction && it.name == name) || (it is KtProperty && it.name == name) ||
                        (it is KtEnumEntry && it.name == name)
                }) return true
    ...
    while (node != null) {
        if (node is KtBlockExpression) {
            for (st in node.statements) {
                if (st is KtClassOrObject && st !is KtEnumEntry &&
                    st.fqName == null && st.name != null
                ) out.getOrPut(st.name!!) { dev.ide.lang.kotlin.symbols.SourceIndexBuilder.localTypeFqn(st) }
            }
        }
        node = node.parent
    }
...
internal fun enclosingClassFqnOf(element: KtElement): String? {
    var node: KtElement? = element
    while (node != null) {

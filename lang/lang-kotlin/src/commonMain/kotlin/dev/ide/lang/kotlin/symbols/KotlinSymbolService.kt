...
class KotlinSymbolService(
...
) : KotlinTypeContext, AutoCloseable {
    ...
    private val stdlibJar = bundledStdlibJarPath()
    // Paths are strings here, and down in ClasspathReader: `java.nio.file.Path` cannot be named from common
    // code, and a path is a string on every platform that has one.
    ...
    private val javaShapeCache = ConcurrentMap<String, Holder<JavaShape>>()
    ...
    private val fileCache = ConcurrentMap<String, CachedFile>()

    // Guards the source model and the per-file version map: the instance monitor these blocks used to take,
    // named, because common Kotlin has no `synchronized`. Reentrant, as a monitor is — [model] is reached
    // from inside a guarded section.
    ...
    // Keyed by declaration IDENTITY, which a plain map gives here: `RawCallable` declares no `equals`, so
    // two callables compare equal only when they are the same object. An identity map was the JVM spelling
    // of that, not a different guarantee.
    ...
    private val inferringMemberOwners = ThreadLocalValue { HashMap<String, Int>() }
    ...
    private val classpathSupertypeMemo = ConcurrentMap<String, List<String>>()
    @Volatile
    private var sourceSupertypeMemo = ConcurrentMap<String, List<String>>()
    ...
    private val supertypeArgTemplateMemo = ConcurrentMap<String, List<TypeRef>>()
    ...
    @Volatile
    private var sourceSupertypeArgTemplateMemo = ConcurrentMap<String, List<TypeRef>>()
    ...
    private val classpathExtMemo = ConcurrentMap<String, List<KotlinSymbol>>()
    ...
    private val checkMembersMemo = ConcurrentMap<String, List<KotlinSymbol>>()
    ...
    private val companionMembersMemo = ConcurrentMap<String, List<KotlinSymbol>>()
    ...
    private val mappedStaticsMemo = ConcurrentMap<String, List<KotlinSymbol>>()
    ...
    private val classpathTypeExistsMemo = ConcurrentMap<String, Boolean>()
    ...
    private val classpathShapeMemo = ConcurrentMap<String, Holder<TypeShape>>()
    private val builtinShapeMemo = ConcurrentMap<String, Holder<TypeShape>>()
    ...
    private val topLevelLibMemo = ConcurrentMap<String, List<KotlinSymbol>>()
    private val topLevelBuiltinMemo = ConcurrentMap<String, List<KotlinSymbol>>()
    ...
    private val optInLevelCache = ConcurrentMap<String, Holder<OptInLevel>>()
    private val optInScanCache = ConcurrentMap<String, Holder<KotlinOptIn.OptInScan>>()
    ...
    fun optInLevelOf(annotationFqn: String): OptInLevel? =
        optInLevelCache.getOrPut(annotationFqn) {
            ...
            Holder(level?.let { if (it == "WARNING") OptInLevel.WARNING else OptInLevel.ERROR })
        }.value
    ...
    private fun optInScanOf(classFqn: String): KotlinOptIn.OptInScan? =
        optInScanCache.getOrPut(classFqn) {
            Holder(reader.classBytes(classFqn)?.let { KotlinOptIn.scan(it) })
        }.value
    ...
    fun setOverlay(map: Map<String, String>) {
        stateLock.withLock {
            val old = overlay
            if (map == old) return@withLock // identical content → the model already reflects it
    ...
    fun externalContentStamp(exceptPath: String): Long = stateLock.withLock {
        var h = 0L
    ...
    private fun model(): ModuleSourceModel =
        cachedModel ?: stateLock.withLock {
    ...
    fun typeFromText(text: String?, ctx: FileContext?, enclosingClassFqn: String? = null): KotlinType? {
        ...
        if (argText.isBlank() && aliasExpandDepth.get().value < ALIAS_EXPAND_LIMIT) {
     ...
     *  A mutable box, because a per-thread value is read here and written back in place. */
    private class Depth { var value = 0 }
    private val aliasExpandDepth = ThreadLocalValue { Depth() }
    ...
    fun resolveTypeName(name: String, ctx: FileContext?, enclosingClassFqn: String? = null): String? {
        ...
        if (typeNameMemo === memo) memo[key] = Holder(computed)
        return computed
    }
    ...
    private fun resolveTypeNameUncached(simple: String, ctx: FileContext?, enclosingClassFqn: String?): String? {
        ...
        // 1. A NESTED SOURCE type reached by SIMPLE name from within an enclosing class (`Plan` inside
        //    `class Game { private class Plan }`, or a sibling nested type). Kotlin scopes a classifier
        //    INNERMOST-OUT, so this comes before every file-level step below: a nested type SHADOWS a
        ...
        //    the object read as a classifier used as a value.
        //
        ...
        //    step 9 (below), where it is reached only if nothing else resolved: an enclosing class taken from
        ...
        // 2. A SAME-PACKAGE source class of that name, when nothing imports the name explicitly. The built-ins
        //    below reach a file through the OUTERMOST scope there is -- the implicit `kotlin.*` /
        //    `kotlin.collections.*` star imports -- so anything declared nearer than that shadows them, and a
        //    file's own package is nearer. Guarded on the absence of an explicit import because an explicit
        //    import does outrank the package (and step 4's note below depends on still reaching that case).
        //
        //    `Iterator`, `Comparator`, `Function`, `Number`, `Enum`, `Annotation` and `Throwable` are all
        //    names an ordinary project declares. Unshadowable, they resolved to the built-in and every use
        //    was checked against the wrong type: the stdlib's own `UByteArray` -- which declares a nested
        //    `class Iterator` and calls `Iterator(storage)` -- was told it could not instantiate an interface.
        val explicitImportFqn =
            ctx?.imports?.firstOrNull { !it.isStar && it.simpleName == simple }?.fqn
        if (explicitImportFqn == null) {
            val pkg = ctx?.packageName ?: ""
            val own = if (pkg.isEmpty()) simple else "$pkg.$simple"
            if (own in model().classByFqn) return own
        }
        // 3. Kotlin built-in simple types (String/Int/List/Map/…). They are in scope everywhere via the
        //    implicit `kotlin.*`/`kotlin.collections.*` imports, and are NOT displaced by an explicit import
        //    that happens to share the simple name (e.g. `import icons.automirrored.filled.List` → the icon
        //    object must not displace `kotlin.collections.List` in type-annotation position). What does
        //    displace them is a nearer DECLARATION, which is why steps 1 and 2 run first.
        Builtins.DEFAULT_SIMPLE_TYPES[simple]?.let { return it }
        // 4. An explicit (non-star) import. It wins when it resolves to a KNOWN type. When the imported FQN is
        ...
        // 5. The file's own package (source, then classpath) — a same-package type needs no import.
        ...
        // 6. A SAME-PACKAGE project SOURCE class by simple name — needs no import. Kotlin sources come from the
        //    model (step 5 also covers these when the file has a package directive); Java sources from the index
        ...
        // `$samePkg.$simple`, which step 5 above has already tested, so the only case left for the scan to find
        ...
        // 7. A top-level synthetic class (Android `R`/`BuildConfig`, a ViewBinding) by simple name; nested
        ...
        //    import anywhere else. An explicit import already resolved it at step 4 (a synthetic FQN satisfies
        //    [isKnownType]), so what remains here is the same-package case and a star import of its package,
        //    which the step-8 loop below cannot cover, since a synthetic has no type shape to probe.
        ...
        // 8. A star-imported package, then Kotlin's implicit default star imports (kotlin.*, java.lang, …):
        ...
        // 9. A nested type in the enclosing chain that has a compiled type SHAPE but no source class (step 1
        ...
        // 10. An explicit import whose target is not a known type and that nothing local shadowed: return
        //     its FQN so the unresolved-type/import diagnostic still points at the intended name (the pre-fix
        //     behaviour for the no-conflict case — a missing import with no same-named local declaration).
    ...
    private fun typeShape(fqn: String): TypeShape? {
        ...
        classpathShapeMemo[fqn]?.let { return it.value }
        val computed = typeShape(fqn, aliasDepth = 0)
        classpathShapeMemo[fqn] = Holder(computed)
        return computed
    }
    ...
    private fun builtinShape(fqn: String): TypeShape? {
        ...
        builtinShapeMemo[fqn]?.let { return it.value }
        val computed = idx.exact<TypeShape>(BUILTINS, fqn).firstOrNull()?.withContext(this)
        builtinShapeMemo[fqn] = Holder(computed)
        return computed
    }
     ...
     *  `Iterable<Iterable<T>>` vs `List<List<Int>>` binds `T = Int`. First binding wins. */
    private fun unifyReceiverArg(
        ...
        if (declared.isTypeParameter) {
            out.getOrPut(declared.qualifiedName) { actual }; return
        }
    ...
    internal fun receiverSupertypeArgs(
        ...
        val key = "$subFqn $superFqn"
        // Not `getOrPut`: a non-supertype must NOT be cached, and bailing out of the computation needs a
        // `return` that crosses the lambda, which only an inline call allows. `run` is one; `getOrPut` on
        // the portable map is not.
        val template = memo[key] ?: run {
            val paramRefs = params.map { KotlinType(it, isTypeParameter = true, context = this) }
            val walked = walkSupertypeArgs(subFqn, paramRefs, superFqn, HashSet())
                ?: return null // don't cache a non-supertype
            walked.also { memo[key] = it }
        }
    ...
    private fun topLevelLibScan(idx: IndexQueries, name: String): List<KotlinSymbol> =
    ...
    private fun topLevelBuiltinScan(idx: IndexQueries, name: String): List<KotlinSymbol> =
    ...
    private fun inferReturnFromBody(rc: RawCallable, ownerFqn: String? = null): KotlinType? {
        ...
        val result = try {
            ...
            if (isResourceExhaustion(t)) throw t
            null
        } finally {
            guard.remove(rc)

...
class KotlinIntroduceVariableActionProvider : ActionProvider {
    ...
    private fun isExtractable(e: KtExpression): Boolean = when (e) {
        ...
        -> false
        else -> e !is KtNameReferenceExpression &&
            e !is KtConstantExpression &&
...
class KotlinBracesActionProvider : ActionProvider {
    ...
    private fun enclosingBranchBody(ctx: EditorActionContext): Branch? {
        ...
        var n: KtElement? = ctx.node.psiOrNull() ?: return null
        while (n != null) {
            when (val owner = n) {
                is KtIfExpression -> {
...
private fun DomNode.psiOrNull(): KtElement? = (this as? KotlinDomNode)?.psi

/** The DOM node wrapping [psi], so a fix computed over PSI can still speak the neutral currency. */
private fun EditorActionContext.nodeFor(psi: KtElement): DomNode? {
...
private inline fun <reified T : KtElement> EditorActionContext.nearestPsi(): T? {
    var n: KtElement? = node.psiOrNull()
    while (n != null) {
        if (n is T) return n
        n = n.parent
    }

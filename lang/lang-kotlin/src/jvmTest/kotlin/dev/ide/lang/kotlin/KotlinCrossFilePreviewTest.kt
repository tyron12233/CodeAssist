...
class KotlinCrossFilePreviewTest {
    ...
    private fun serviceOver(files: Map<String, String>): Pair<KotlinSymbolService, Path> {
        ...
        return KotlinSymbolService(listOf(DiskFile(dir)), listOf(stdlibJarPath().toString())) to dir
    }
    ...
    fun pullsADataClassAndHelperFromAnotherModule() {
        ...
        val appService = KotlinSymbolService(listOf(DiskFile(appDir), DiskFile(coreDir)), listOf(stdlibJarPath().toString()))
        val coreService = KotlinSymbolService(listOf(DiskFile(coreDir)), listOf(stdlibJarPath().toString()))

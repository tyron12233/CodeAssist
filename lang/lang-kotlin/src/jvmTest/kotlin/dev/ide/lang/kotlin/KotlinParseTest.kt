...
class KotlinParseTest {
    ...
    fun controlFlowAndDeclarationStructureAreNamedKinds() {
        ...
        val expected = listOf(
            ...
            // A doc comment is ONE token to the file parse, so these two come from a sub-parse of its text
            // that the DOM splices in (`kdocAwareChildren`). They are here because they are the part of a KDoc
            // the IDE treats as code: `[Foo.bar]` is a reference it resolves and navigates.

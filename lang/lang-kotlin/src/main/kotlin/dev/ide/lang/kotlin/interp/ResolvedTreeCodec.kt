 ...
 * `RNode`/`Binding`/`ResolvedCallable`/[KotlinType] closure. The model is pure data (no PSI), so a lowered
 * declaration round-trips losslessly; `KotlinType.context` (a private, transient resolution callback) is
 * intentionally dropped and rebound locally after transport.
 *
 * Two consumers share it: the Compose preview WIRE blob that crosses into the `:preview` process
 ...
 * overload-resolution fix, a new desugaring): the wire blob never crosses process versions (IDE and `:preview`
 * ship in one APK), but a disk-cache entry OUTLIVES the app version that wrote it, and a stale-format entry
 * must be discarded rather than decoded into a subtly-wrong preview.
 */
object ResolvedTreeCodec {
    ...
    class Writer(val d: DataOutputStream) {
        ...
        fun function(f: ResolvedFunction) {
            ...
            // The old in-place wire codec MISSED this field, so the `:preview` process decoded every top-level
            // `val` as re-evaluated-per-read — breaking CompositionLocal identity (custom themes) remotely.
            bool(f.singletonBackingField)
        }

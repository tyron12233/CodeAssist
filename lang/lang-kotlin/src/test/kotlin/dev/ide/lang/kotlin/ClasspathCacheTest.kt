...
class ClasspathCacheTest {
    ...
    fun extensionScanPersistsAndReloads() = withTempDir("kxt-cache") { cacheDir ->
        ...
        ClasspathReader(listOf(jar.toString()), cacheDir.toString()).use { first ->
        ...
        ClasspathReader(listOf(jar.toString()), cacheDir.toString()).use { second ->
    ...
    fun nonKotlinJarIsSkippedCheaply() {
        ...
        ClasspathReader(listOf(emptyJar.toString())).use { r ->

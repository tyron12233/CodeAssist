 ...
 * subscriber receives a published event, with a background activity reading through the model lock
    ...
    fun pluginRegistersExtensionAndReceivesEvents() {
        runBlocking {
            ...
            try {
                ...
                // 3) a background activity reads through the model lock and reports progress
                val count = platform.activityManager.launch(ActivitySpec("verify")) {
                    progress.report(1.0, "ok")
                    readAction { platform.extensions.extensions(ep).size }
                }.await()

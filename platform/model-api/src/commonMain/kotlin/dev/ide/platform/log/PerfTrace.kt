...
object PerfTrace {
    ...
    private val syspropSeed: Boolean = perfTraceLaunchFlag()
...
/**
 * The launcher's `-D` seed for [PerfTrace], or false where there is no launcher.
 *
 * System properties are a JVM notion. An iOS app is started by the system with no command line to read, so
 * the flag can only ever come from the persisted user preference there.
 */
internal expect fun perfTraceLaunchFlag(): Boolean

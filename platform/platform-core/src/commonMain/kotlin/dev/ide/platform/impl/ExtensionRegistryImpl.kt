...
class ExtensionRegistryImpl(private val parent: ExtensionRegistry? = null) : ExtensionRegistry {
    ...
    private val byPoint = ConcurrentMap<String, CopyOnWriteList<Registration>>()

    override fun <T : Any> register(ep: ExtensionPoint<T>, impl: T, plugin: PluginId): Disposable {
        val list = byPoint.getOrPut(ep.id) { CopyOnWriteList() }
    ...
    override fun <T : Any> extensions(ep: ExtensionPoint<T>): List<T> {
        val own = byPoint[ep.id]?.snapshot()?.map { it.impl as T } ?: emptyList()
    ...
    override fun <T : Any> contributions(ep: ExtensionPoint<T>): List<Contribution<T>> {
        val own = byPoint[ep.id]?.snapshot()?.map { Contribution(it.impl as T, it.plugin) } ?: emptyList()

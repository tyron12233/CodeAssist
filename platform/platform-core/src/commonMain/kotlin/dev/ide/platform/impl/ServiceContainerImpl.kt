...
class ServiceContainerImpl(
    ...
    private class Holder(val factory: ServiceFactory<*>) {
        /** Serializes construction of THIS service, so its factory runs once. Not container-wide. */
        val lock = Lock()
    ...
    private val entries = ConcurrentMap<String, Holder>()
    private val programmatic = ConcurrentMap<String, ServiceFactory<*>>()
    private val disposer = CompositeDisposable()

    /**
     * Instances already handed to [disposer], by IDENTITY. See [own].
     *
     * A list scanned with `===` rather than a hash set, because identity hashing is a JVM facility with no
     * common counterpart, and a service is not obliged to leave `equals`/`hashCode` alone — which is exactly
     * what a hash set would call. The list holds one entry per disposable service in this scope, so the scan
     * is over a handful of elements on a path that runs once per service constructed.
     */
    private val owned = CopyOnWriteList<Disposable>()

    /** Guards the check-then-add in [own]; the list itself is safe, the PAIR of operations is not. */
    private val ownLock = Lock()
    ...
    override fun <T : Any> getServiceOrNull(key: ServiceKey<T>): T? {
        ...
        if (factory != null) {
            val holder = entries.getOrPut(id) { Holder(factory) }
    ...
    override fun <T : Any> registerServiceIfAbsent(key: ServiceKey<T>, factory: ServiceFactory<T>) {
        ...
        programmatic.getOrPut(key.id) { factory }
    }
    ...
    private fun own(instance: Any) {
        ...
        val isNew = ownLock.withLock {
            if (owned.snapshot().any { it === disposable }) {
                false
            } else {
    ...
    private fun build(id: String, holder: Holder): Any {
        ...
        // `withLock` is not inline (it crosses an expect/actual boundary), so this body returns to the
        // lambda rather than out of the method.
        return holder.lock.withLock {
            holder.instance ?: run {
    ...
    companion object {
        ...
        private val constructionStack = ThreadLocalValue { ArrayDeque<String>() }
    }
}

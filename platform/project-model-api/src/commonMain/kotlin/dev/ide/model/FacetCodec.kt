...
class FacetCodecRegistry(private val extensions: ExtensionRegistry) {
    ...
    private val reportedTables = ConcurrentMap<String, Boolean>()
    ...
    fun register(codec: FacetCodec<*>, plugin: PluginId = DEFAULT_PLUGIN): FacetCodecRegistry {
        require(codec.tomlTable !in RESERVED_FACET_TABLES) {
            "facet codec ${codec::class.simpleName} claims '${codec.tomlTable}', " +
    ...
    private fun reportShadowed(table: String) {
        if (reportedTables[table] != null) return
        reportedTables[table] = true
        // The simple name, not the qualified one: `KClass.qualifiedName` is unavailable off the JVM, and the
        // message names codecs so a reader can find them, not so a tool can parse them.
        val claimants = codecs.filter { it.tomlTable == table }.map { it::class.simpleName }
        log.warn(

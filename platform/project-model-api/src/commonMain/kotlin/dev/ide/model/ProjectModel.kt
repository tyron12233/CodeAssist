...
class DependencyScope(
    ...
    companion object {
         ...
         */
        // `synchronized` is a JVM intrinsic; the portable lock stands where it did, guarding the same map.
        private val knownLock = Lock()
        ...
        fun register(scope: DependencyScope): DependencyScope {
            knownLock.withLock { known[scope.name] = scope }
            return scope
        }

        /** Every registered scope: the built-ins first, then plugin-defined ones in registration order. */
        fun registered(): List<DependencyScope> = knownLock.withLock { known.values.toList() }
        ...
        fun byId(id: String): DependencyScope? = knownLock.withLock { known.values.firstOrNull { it.id == id } }
    }
}
...
interface ClasspathSnapshot {
    ...
    private class Snapshot(override val entries: List<ClasspathEntry>) : ClasspathSnapshot {
        // The same bytes through the same SHA-256, so a fingerprint an earlier build persisted still
        // compares equal: per entry the kind id, a NUL, the root path, and a newline.
        override fun fingerprint(): ContentHash {
            val bytes = ArrayList<Byte>()
            for (e in entries) {
                e.kind.id.encodeToByteArray().forEach(bytes::add)
                bytes.add(0)
                e.root.path.encodeToByteArray().forEach(bytes::add)
                bytes.add('\n'.code.toByte())
            }
            return ContentHash.of(bytes.toByteArray())
        }

...
import dev.ide.platform.relativizePath
...
class ProjectModelStore internal constructor(
    /**
     * The workspace root, absolute and normalized.
     *
     * A string, not a `java.nio.file.Path`, because the model has to be nameable from common code — that
     * one type is what kept the whole of it on the JVM. A JVM caller that wants a `Path` still writes
     * `store.rootPath`: see the extension in `JvmPaths.kt`.
     */
    val root: String,
    ...
    private val moduleContainers = ConcurrentMap<ModuleId, ServiceContainer>()
    ...
    fun moduleContainer(id: ModuleId): ServiceContainer {
        ...
        return moduleContainers.getOrPut(id) {
            ServiceContainerImpl(
    ...
    internal fun resolveRel(base: String, rel: String): String = resolvePath(base, rel)

    internal fun projectRoot(p: ProjectData): String = resolveRel(root, p.rootRelPath)

    internal fun fileFromWorkspace(rel: String): VirtualFile = vfs.fileFor(resolveRel(root, rel))

    internal fun relativizeToWorkspace(file: VirtualFile): String = relativizePath(root, file.path)
}

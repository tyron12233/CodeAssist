...
class FacetDataWriteTest {
    ...
    fun aValueTomlCannotHoldIsRefusedWhereItIsStaged() = withModule { module ->
        ...
        // The SIMPLE name: `::class.java.name` has no common counterpart, and the model is multiplatform
        // now. "StringBuilder has no TOML representation" names the offending value just as clearly.
        assertTrue("StringBuilder" in wrongType.message.orEmpty(), wrongType.message.orEmpty())
    ...
    fun aSecondCodecForATableIsReportedOnce() {
        ...
        try {
            ...
            // Simple names, not qualified ones: `KClass.qualifiedName` does not exist off the JVM, and the
            // message names the codecs so a reader can find them rather than so a tool can parse them.
            val warning = records.single { "module.toml table 'java'" in it.message }
            assertTrue(JavaFacetCodec::class.simpleName!! in warning.message, warning.message)
            assertTrue(ImpostorJavaCodec::class.simpleName!! in warning.message, warning.message)
        } finally {
            Log.removeSink(sink)
        }
    }

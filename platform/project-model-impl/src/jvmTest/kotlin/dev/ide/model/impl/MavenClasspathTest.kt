...
class MavenClasspathTest {
    ...
    fun dexDedupCollapsesBundledAndMavenStdlib() {
        ...
        val result = dedupeJarsForAndroidDex(listOf(maven, bundled)).map { it.toString() }
    ...
    fun dexDedupKeepsUnversionedDirsAndDistinctArtifacts() {
        ...
        val result = dedupeJarsForAndroidDex(listOf(classesJar, moduleOut, coroutines)).map { it.toString() }
    ...
    fun dexDedupKeepsAndroidAndJvmOfDistinctModules() {
        ...
        val result = dedupeJarsForAndroidDex(listOf(android, jvm)).map { it.toString() }
    ...
    fun dexDedupDropsAMissingBundledStdlibForThePresentMavenOne() {
        ...
        try {
            ...
            val result = dedupeJarsForAndroidDex(listOf(bundledMissing, maven)).map { it.toString() }
            assertEquals(

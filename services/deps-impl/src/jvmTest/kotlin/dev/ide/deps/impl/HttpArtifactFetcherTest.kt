...
class HttpArtifactFetcherTest {
    ...
    fun fetchToStreamsFullBodyToDisk() {
        withTempDir("fetchto") { dir ->
            ...
            assertTrue(fetcher.fetchTo("$base/ok", dest.toString()))
    ...
    fun fetchToReturnsFalseOn404() {
        withTempDir("fetchto404") { dir ->
            val dest = dir.resolve("artifact.bin")
            assertFalse(fetcher.fetchTo("$base/missing", dest.toString()))
        }
    }
    ...
    fun fetchToFollowsRedirect() {
        withTempDir("fetchtoredir") { dir ->
            val dest = dir.resolve("artifact.bin")
            assertTrue(fetcher.fetchTo("$base/redirect", dest.toString()))
    ...
    fun fetchThrowsOn403() {
        ...
        assertFailsWith<DepsHttpException> { fetcher.fetch("$base/forbidden") }
    }

    @Test
    fun fetchToThrowsOn403() {
        withTempDir("fetchto403") { dir ->
            assertFailsWith<DepsHttpException> { fetcher.fetchTo("$base/forbidden", dir.resolve("artifact.bin").toString()) }
        }
    }
}

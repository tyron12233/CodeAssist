...
object StoreAuth {
    /**
     * The mobile redirect, for both phone hosts.
     *
     * Android receives it as a deep link through its intent-filter; iOS receives it as the callback of an
     * `ASWebAuthenticationSession`. One value rather than one per platform, because what has to agree with
     * it is Supabase's `additional_redirect_urls`, which is a list of literal URLs — a second entry would
     * be a second thing to remember to add.
     */
    const val MOBILE_REDIRECT = "codeassist://auth-callback"

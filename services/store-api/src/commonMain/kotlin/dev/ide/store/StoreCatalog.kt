...
interface StoreCatalogSource {
    ...
    /**
     * Download an approved payload from the public bucket to [intoPath].
     *
     * [intoPath] is a filesystem path rather than a `java.io.File`: this contract is implemented on the
     * JVM, on ART and on iOS, and only one of those has that type.
     *
    ...
    /**
     * Download a public media object (a screenshot) to [intoPath].
     *
     ...
     */
    fun downloadMedia(storagePath: String, intoPath: String): StoreResult<Unit> =
        StoreResult.Unavailable("No store endpoint")
    ...
    companion object {
        ...
        val Unconfigured: StoreCatalogSource = object : StoreCatalogSource {
            ...
            override fun downloadMedia(storagePath: String, intoPath: String) =
                StoreResult.Unavailable<Unit>("No store endpoint")
            override fun downloadPayload(storagePath: String, expectedSha256: String?, expectedBytes: Long, intoPath: String, onProgress: (Float) -> Unit) =
                StoreResult.Unavailable<Unit>("No store endpoint")

...
object StoreFeedMapper {
     ...
     */
    fun itemToUi(item: RemoteStoreItem): UiStoreItem = item.toUi(emptyMap())
     ...
     */
    fun itemToUi(item: RemoteStoreItem, bundled: Map<String, UiStoreItem>): UiStoreItem =
        item.toUi(bundled)
 ...
 *
 * A seam because `Dispatchers.IO` does not exist on Kotlin/Native: coroutines declares it internal there,
 * so the iOS actual is `Dispatchers.Default`. That is the right answer rather than a compromise — the
 * blocking work here is a socket the OS is already multiplexing, not a thread pool this module should be
 * sizing for itself.
 */
expect val storeIo: kotlinx.coroutines.CoroutineDispatcher

 ...
 */
class StoreInstallHistory(private val file: () -> String?) {
    ...
    fun installedPaths(): Map<String, String> = entries()
        .mapNotNull { (id, path) -> path?.takeIf { StoreFs.isDirectory(it) }?.let { id to it } }
        .toMap()
    ...
    fun remember(id: String, rootPath: String? = null) {
        ...
        parentPath(target)?.let { StoreFs.mkdirs(it) }
        StoreFs.writeText(
            target,
    ...
    private fun entries(): List<Pair<String, String?>> =
        file()?.takeIf { StoreFs.isFile(it) }
            ?.let { StoreFs.readText(it) }
            ?.lineSequence()

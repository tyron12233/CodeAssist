...
class StoreInstaller(
    ...
    fun install(
        ...
        val archive = StoreFs.tempPath("ca-store-", ".zip")
        try {
            ...
            return when (val extracted = extractor.extract(archive, projectsRoot, payload.title)) {
                is StoreResult.Ok -> {
                    ...
                    report(UiInstallState.INSTALLED, 1f, rootPath = dir)
                    UiStoreInstallResult(true, "Added to your projects", dir)
                }
                is StoreResult.Unavailable -> failed(extracted.reason)
                is StoreResult.Failed -> failed(extracted.message)
            }
        } finally {
            StoreFs.delete(archive)
        }
    }
}

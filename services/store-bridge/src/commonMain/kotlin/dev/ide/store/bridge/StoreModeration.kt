 ...
 */
class StoreModeration(private val moderation: StoreModerationService) {
     ...
     *
     * Guarded rather than concurrent-by-type: `ConcurrentHashMap` is JVM-only and this runs on iOS too.
     * The queue is read on one dispatcher and acted on from the UI, so the two do meet.
     */
    private val known = HashMap<String, PendingSubmission>()
    private val knownLock = dev.ide.store.impl.platform.StoreLock()
    ...
    fun approve(versionId: String, note: String?, clearIconIfMissing: Boolean): String? {
        val submission = knownLock.withLock { known[versionId] } ?: return STALE
    ...
    /** Fetch a private submission image to [intoPath]. The caller owns the cache path and its naming. */
    fun downloadSubmissionImage(storagePath: String, intoPath: String): Boolean =
        moderation.downloadObject("store-uploads", storagePath, intoPath) is StoreResult.Ok
    ...
    internal companion object {
         ...
         */
        internal fun parseInstantMs(iso: String?): Long = parseIsoMillis(iso)
    }
}

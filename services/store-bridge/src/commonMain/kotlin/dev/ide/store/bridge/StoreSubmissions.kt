...
class StoreSubmissions(
     ...
     */
    private val packed = HashMap<String, PackagedProject>()

    /** The last packaging failure per project, for the screen to show. */
    private val packErrors = HashMap<String, String>()

    /**
     * Guards both maps.
     *
     * `ConcurrentHashMap` is JVM-only and this module runs on iOS too. The maps are written by a packaging
     * call and read by the submit that follows it, which are different coroutines on the same dispatcher.
     */
    private val packLock = dev.ide.store.impl.platform.StoreLock()
    ...
    fun pack(rootPath: String): UiPackagedProject? =
        when (val result = submissions.pack(rootPath)) {
            ...
            is StoreResult.Unavailable -> { packLock.withLock { packErrors[rootPath] = result.reason }; null }
            is StoreResult.Failed -> { packLock.withLock { packErrors[rootPath] = result.message }; null }
        }

    /** Why the last [pack] of [rootPath] failed, or null if it succeeded or was never tried. */
    fun packError(rootPath: String): String? = packLock.withLock { packErrors[rootPath] }
    ...
    fun submit(draft: UiSubmissionDraft, packaged: UiPackagedProject): UiSubmitResult {
        ...
        val archive = packLock.withLock { packed[packaged.rootPath] }
            ?: when (val again = submissions.pack(packaged.rootPath)) {
                is StoreResult.Ok -> again.value.also { packLock.withLock { packed[packaged.rootPath] = it } }
    ...
    private fun iconFileFor(bytes: ByteArray?): String? {
        ...
        val file = dev.ide.store.impl.platform.StoreFs.tempPath(
            "ca-store-icon-",
            ".${imageExtension(bytes)}",
        )
        return if (dev.ide.store.impl.platform.StoreFs.writeBytes(file, bytes)) file else null
    }
    ...
    /**
     * [account] with what the publisher row knows about it filled in.
     *
     * A session says who you are and nothing else: the handle, the display name and — the one that
     * decides whether a surface exists at all — whether this account moderates the store are all on the
     * publisher row. The profile read is the only round trip that already happens on sign-in, so it
     * answers all of them and the app needs no second call.
     *
     * Shared rather than written per host because forgetting one field here is silent and specific: drop
     * [StoreAccount.isAdmin] and a real moderator is told the store will not let them moderate; drop the
     * handle and their own profile renders nameless. Each host adds only what is genuinely its own (the
     * Android one binds its push device first), and the mapping is decided once.
     */
    fun adopt(account: dev.ide.store.StoreAccount): dev.ide.store.StoreAccount {
        val profile = profile() ?: return account
        return account.copy(
    ...
    companion object {
         ...
         */
        fun newestPerItem(subs: List<UiStoreSubmission>): Map<String, String?> =

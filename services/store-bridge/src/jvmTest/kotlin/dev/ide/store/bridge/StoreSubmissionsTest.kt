...
class StoreSubmissionsTest {
    ...
    /**
     * A session says who you are and nothing else. Everything the app draws about an account — the name,
     * the handle, and whether the moderation surface exists at all — comes off the publisher row, and this
     * is the one place that folds it in.
     *
     * The `isAdmin` case is here because losing it is silent and specific: a real moderator signs in and
     * is told the store will not let them moderate, which reads like the backend refused them.
     */
    @Test
    fun adoptFillsInWhatThePublisherRowKnows() {
        val submissions = StoreSubmissions(
            FakeSubmissions(
                packResult = StoreResult.Ok(archive()),
                profileResult = StoreResult.Ok(
        ...
        val adopted = submissions.adopt(StoreAccount(userId = "u1", email = "t@example.invalid"))

        assertEquals("tyron", adopted.handle)
        assertEquals("Tyron Scott", adopted.displayName)
        assertEquals("https://example.invalid/a.png", adopted.avatarUrl)
        assertTrue(adopted.verified)
        assertTrue(adopted.isAdmin, "a moderator's session has to carry that, or the surface never appears")
        assertEquals("u1", adopted.userId, "the session's own identity is not overwritten")
    }

    /** The provider's picture stands when the publisher row has none of its own. */
    @Test
    fun adoptKeepsTheProviderAvatarWhenTheRowHasNone() {
        val submissions = StoreSubmissions(
            FakeSubmissions(
                packResult = StoreResult.Ok(archive()),
                profileResult = StoreResult.Ok(
                    StorePublisherProfile(handle = "tyron", displayName = "Tyron Scott"),
                ),
            ),
        )

        val adopted = submissions.adopt(
            StoreAccount(userId = "u1", avatarUrl = "https://github.invalid/me.png"),
        )

        assertEquals("https://github.invalid/me.png", adopted.avatarUrl)
        assertFalse(adopted.isAdmin, "nobody is a moderator by default")
    }

    /** A profile that could not be read leaves the account exactly as the session described it. */
    @Test
    fun adoptLeavesTheAccountAloneWhenTheProfileCannotBeRead() {
        val submissions = StoreSubmissions(
            FakeSubmissions(
                packResult = StoreResult.Ok(archive()),
                profileResult = StoreResult.Unavailable("offline"),
            ),
        )

        val account = StoreAccount(userId = "u1", handle = "known", isAdmin = true)
        assertEquals(account, submissions.adopt(account))
    }
    ...
    private class FakeSubmissions(
        ...
        private val profileResult: StoreResult<StorePublisherProfile?> = StoreResult.Ok(null),
        ...
        override fun myProfile(): StoreResult<StorePublisherProfile?> = profileResult

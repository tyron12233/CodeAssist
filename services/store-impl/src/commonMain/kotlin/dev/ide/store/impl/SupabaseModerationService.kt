...
import dev.ide.store.impl.platform.storeLog
...
class SupabaseModerationService(
    ...
    override fun approve(
        ...
        val publishedShots = submission.screenshotPaths.mapIndexedNotNull { index, path ->
            ...
            when (val r = copyObject(UPLOADS, path, MEDIA, destination, token)) {
                ...
                is StoreResult.Unavailable -> { storeLog("StoreModeration", "Screenshot $index not published: ${r.reason}"); null }
                is StoreResult.Failed -> { storeLog("StoreModeration", "Screenshot $index not published: ${r.message}"); null }
            }
        }
        ...
        submission.iconPath?.let { source ->
            ...
            when (val r = copyObject(UPLOADS, source, MEDIA, destination, token)) {
                ...
                is StoreResult.Unavailable -> { storeLog("StoreModeration", "App icon not published: ${r.reason}"); keepIcon = true }
                is StoreResult.Failed -> { storeLog("StoreModeration", "App icon not published: ${r.message}"); keepIcon = true }
            }
        }
    ...
    override fun downloadObject(bucket: String, storagePath: String, intoPath: String): StoreResult<Unit> {
        if (!configured) return StoreResult.Unavailable("No store endpoint configured")
        val token = accounts.bearer() ?: return StoreResult.Failed("Sign in first")
        val url = "$base/storage/v1/object/$bucket/$storagePath"
        var reply = http.download(url, authHeaders(token), intoPath)
        if (reply.status == 401) {
            // Same expired-session retry the JSON calls make. This one is worth making by hand because a
            // review fetches the archive after reading a queue that may be hours old.
            val fresh = accounts.reauthorize(token)
                ?: return StoreResult.Failed("Could not read that file (HTTP 401)", 401)
            reply = http.download(url, authHeaders(fresh), intoPath)
        }
        // A half-written file would decode as garbage, so it does not survive a failure; the transport
        // deletes what it could not finish writing.
        reply.error?.let { return StoreResult.Unavailable(it) }
        if (reply.status !in 200..299) {
            StoreFs.delete(intoPath)
            return StoreResult.Failed("Could not read that file (HTTP ${reply.status})", reply.status)
        }
    ...
    private fun deleteObject(bucket: String, key: String, token: String) {
        http.request("DELETE", "$base/storage/v1/object/$bucket/$key", authHeaders(token))
    }

    /** The public bucket serves an approved payload with no auth, so this proves what an install will see. */
    private fun headPublicPayload(publicPath: String): StoreResult<Unit> {
        val r = http.request(
            method = "GET",
            url = "$base/storage/v1/object/public/$PAYLOADS/$publicPath",
            // One byte is all the proof needed, and a 5 MB archive is not worth downloading for it.
            headers = mapOf("Range" to "bytes=0-0"),
        )
        r.error?.let { return StoreResult.Unavailable(it) }
        return if (r.status in 200..299) StoreResult.Ok(Unit) else StoreResult.Failed("HTTP ${r.status}", r.status)
    }
    ...
    private fun authHeaders(token: String): Map<String, String> =
        mapOf("apikey" to apiKey, "Authorization" to "Bearer $token")
    ...
    private fun send(method: String, url: String, body: String?, token: String): StoreResult<String> {
        val headers = buildMap {
            putAll(authHeaders(token))
            if (body != null) put("Content-Type", "application/json")
        }
        val r = http.request(method, url, headers, body)
        return when {
            ...
            r.status == 401 -> StoreResult.Failed("Sign in to moderate", r.status)
            r.status == 403 -> StoreResult.Failed(errorMessage(r.body) ?: "You are not a moderator", r.status)
            r.status == 429 || r.status >= 500 -> StoreResult.Unavailable("Store unavailable (HTTP ${r.status})")
            else -> StoreResult.Failed(errorMessage(r.body) ?: "The store rejected that", r.status)
        }
    }

...
import dev.ide.store.impl.platform.ProjectArchiver
import dev.ide.store.impl.platform.StoreFs
import dev.ide.store.impl.platform.StoreHttp
import dev.ide.store.impl.platform.defaultArchiver
...
class SupabaseSubmissionService(
    ...
    private val packager: ProjectArchiver = defaultArchiver(),
    ...
    override fun submit(
        ...
        val token = accounts.bearer()
            ?: return StoreResult.Failed("Sign in to submit a project")
        val archive = packaged.archivePath
        if (!StoreFs.isFile(archive)) {
            return StoreResult.Failed("The packaged archive is gone; package the project again")
        }
    ...
    private fun uploadScreenshots(
        ...
        if (!StoreFs.isFile(local)) {
            storeLog(TAG, "Screenshot $local is gone; submitting without it")
            return@mapIndexedNotNull null
        }
        val size = StoreFs.size(local)
        if (size > MAX_SCREENSHOT_BYTES) {
            storeLog(TAG, "Screenshot $local is $size bytes, over the $MAX_SCREENSHOT_BYTES limit")
            return@mapIndexedNotNull null
        }
        val extension = local.substringAfterLast('/').substringAfterLast('.', "png").lowercase()
        val objectPath = "$prefix/shot-$index.$extension"
        when (val result = uploadBytes(objectPath, local, mimeFor(extension), token)) {
            is StoreResult.Ok -> objectPath
            is StoreResult.Failed -> {
                storeLog(TAG, "Screenshot upload rejected (HTTP ${result.status}): ${result.message}")
                null
            }
            is StoreResult.Unavailable -> {
                storeLog(TAG, "Screenshot upload failed: ${result.reason}")
                null
            }
        }
    }
    ...
    private fun uploadIcon(prefix: String, path: String?, token: String): String? {
        val local = path ?: return null
        if (!StoreFs.isFile(local)) {
            storeLog(TAG, "App icon $path is gone; submitting without it")
            return null
        }
        val size = StoreFs.size(local)
        if (size > MAX_SCREENSHOT_BYTES) {
            storeLog(TAG, "App icon $path is $size bytes, over the $MAX_SCREENSHOT_BYTES limit")
            return null
        }
        val extension = local.substringAfterLast('/').substringAfterLast('.', "png").lowercase()
        val objectPath = "$prefix/icon.$extension"
        return when (val result = uploadBytes(objectPath, local, mimeFor(extension), token)) {
            is StoreResult.Ok -> objectPath
            is StoreResult.Failed -> {
                storeLog(TAG, "App icon upload rejected (HTTP ${result.status}): ${result.message}")
                null
            }
            is StoreResult.Unavailable -> {
                storeLog(TAG, "App icon upload failed: ${result.reason}")
                null
            }
        }
    }
    ...
    private fun uploadBytes(
        ...
        val r = http.upload(
            method = "POST",
            url = "$base/storage/v1/object/store-uploads/$objectPath",
        ...
        return when {
            ...
            r.status in 200..299 -> StoreResult.Ok(Unit)
            else -> StoreResult.Failed(storageError(r.body) ?: "Upload rejected", r.status)
        }
    }

    private fun upload(objectPath: String, archivePath: String, token: String): StoreResult<Unit> {
        val r = http.upload(
            method = "POST",
            url = "$base/storage/v1/object/store-uploads/$objectPath",
            headers = uploadHeaders(token),
        ...
        return when {
            ...
            r.status == 429 || r.status >= 500 ->
                StoreResult.Unavailable("Upload service unavailable (HTTP ${r.status})")
            else -> StoreResult.Failed(storageError(r.body) ?: "Upload rejected", r.status)
        }
    }

    private fun uploadHeaders(token: String): Map<String, String> = mapOf(
        "apikey" to apiKey,
    ...
    private fun deleteObject(objectPath: String, token: String) {
        http.request(
            method = "DELETE",
            url = "$base/storage/v1/object/store-uploads/$objectPath",
            headers = mapOf("apikey" to apiKey, "Authorization" to "Bearer $token"),
        )
    }
    ...
    private fun send(
        ...
        return when {
            ...
            else -> StoreResult.Failed(restError(r.body) ?: "Store rejected the request", r.status)
        }
    }
    ...
    companion object {
        private const val TAG = "StoreSubmit"

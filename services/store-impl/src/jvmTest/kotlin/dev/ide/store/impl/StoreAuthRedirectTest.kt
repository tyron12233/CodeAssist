...
class StoreAuthRedirectTest {
    ...
    fun theAndroidRedirectIsBuiltFromTheSchemeAndHost() {
        assertTrue(StoreAuth.MOBILE_REDIRECT == "${StoreAuth.SCHEME}://${StoreAuth.HOST}")
        assertTrue(StoreAuth.isAuthRedirect(StoreAuth.MOBILE_REDIRECT))
    }
}
...
class StoreAuthErrorMessageTest {
    ...
    private fun completeWith(errorDescription: String): String {
        ...
        val result = service.complete(
            "${dev.ide.store.StoreAuth.MOBILE_REDIRECT}#error=server_error&error_description=$encoded",
        )

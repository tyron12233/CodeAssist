...
class TerminalEmulator {
     ...
     */
    public TerminalEmulator(TermSession session,
    ...
    public void updateSize(int columns, int rows) {
        ...
        if (!fastResize) {
            ...
            charAtCursor = screen.getSelectedText(cursorColor, mCursorCol, mCursorRow, mCursorCol,
                    mCursorRow);
    ...
    public void append(byte[] buffer, int base, int length) {
        if (EmulatorDebug.LOG_CHARACTERS_FLAG) {
            Log.d(EmulatorDebug.LOG_TAG,
                    "In: '" + EmulatorDebug.bytesToString(buffer, base, length) + "'");
        }
    ...
    private void doEscLeftSquareBracket(byte b) {
        ...
        switch (b) {
            ...
            case 'M': // Delete Lines
            {
                ...
                mScreen.blockCopy(0, mCursorRow + linesToDelete, mColumns, linesToMove, 0,
                        mCursorRow);
            ...
            case 'P': // Delete Characters
            {
                ...
                mScreen.blockCopy(mCursorCol + charsToDelete, mCursorRow, charsToMove, 1,
                        mCursorCol, mCursorRow);
        ...
        for (int i = 0; i <= mArgIndex; i++) {
            ...
            if (code == 0) { // reset
                ...
                mEffect |= TextStyle.fxBold;
                       ...
                       3) { // italics, but rarely used as such; "standout" (inverse colors) with
                // TERM=screen
                ...
                mForeColor = code - 30;
                       ...
                       i + 2 <= mArgIndex &&
                       mArgs[i + 1] == 5) { // foreground 256 color
                int color = mArgs[i + 2];
                if (checkColor(color)) {
                    mForeColor = color;
                }
                ...
                mBackColor = code - 40;
                       ...
                       i + 2 <= mArgIndex &&
                       mArgs[i + 1] == 5) { // background 256 color
    ...
    private void scroll() {
        //System.out.println("Scroll(): mTopMargin " + mTopMargin + " mBottomMargin " +
        // mBottomMargin);
        mScrollCounter++;
    ...
    private void emit(int c, int style) {
        ...
        if (mInsertMode & width != 0) { // Move character to right one space
            ...
            if (destCol < mColumns) {
                mScreen.blockCopy(mCursorCol, mCursorRow, mColumns - destCol, 1, destCol,
                        mCursorRow);
            }
        }
     ...
     *
     * @param c A char[2] containing either a single UTF-16 char or a surrogate pair to be sent
     *          to the screen.
     */
